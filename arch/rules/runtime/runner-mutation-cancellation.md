...
Остаётся проверить полный путь `submit` → `task.cancel` → `task.result` на
трёх ОС и определить наблюдаемое завершение для зависшего защищённого
раннера без принудительного обрыва критического шага.

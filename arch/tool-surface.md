...
**Результат сейчас:** `data`: три ролевые секции с логическими местоположениями, полнотой, машинной причиной завершения и retryability, ранжированием, счётом и происхождением; `dependencyPending` сохраняет доступные `retryAfterMs` и `state` поставщика по общему с диагностикой словарю (DEC.2026-08-22.PROVIDER-NOT-READY-VOCABULARY), а ход приходит через typed MCP progress (отвечают типизированным `data`)

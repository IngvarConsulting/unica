...
программное имя и путь к файлу; синоним читается только у кандидатов ответа.
...
путь к файлу и разрешается в обе стороны: путь даёт адрес, имя и адрес дают
путь. Каждый кандидат несёт `at`, `kind`, `title`, `path` и `reason`; nearest

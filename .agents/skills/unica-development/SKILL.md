...
кода не требует. Число мест с голым `RefusalCode::ProviderUnavailable`
в production-коде `crates/unica-coder/src` должно совпадать с потолком
в `tests/ci/test_refusal_detail_ratchet.py`: новое место без уточнения
из `RefusalDetail` роняет проверку, а сокращение таких мест требует опустить
потолок в том же изменении.

...
- [ADR-0020: Логические адреса источников вместо файловых путей](0020-logical-source-addressing.md)

Запись в статусе `proposed` ещё не действует и не заменяет принятое решение.
...
- `0019` — назначен предложенной записи в черновом PR #210, но не принят в
  `main`. После решения остановить ту реализацию номер выведен из обращения:
  файла `0019-*.md` в `main` нет, а следующий проект использует ADR-0020.

...
- [ADR-0041: Чтение метаданных не зависит от плана мутации](0041-meta-observation-does-not-depend-on-mutation.md)

Запись в статусе `proposed` ещё не действует и не заменяет принятые решения;
после ревью она либо переводится в `accepted` вместе с реализацией и выведенными
проверяемыми правилами, либо остаётся проектным предложением.

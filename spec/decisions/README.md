...
документа не могут разойтись в формулировке (`INV-DOC-SINGLE-RULE-OWNER`).
...
перехода. Последний переход держится не только на тесте: тест реестра
доказывает, что названная проверка исполняется в CI, а что она проверяет именно
это следствие — предмет ревью.
...
которое удерживает `INV-DOC-SUPERSEDE-NOT-EDIT`:
...
- Номер израсходован в момент назначения и не выдаётся повторно, что бы ни
  случилось с записью, которая его занимала (`INV-DOC-NO-ID-REUSE`).
...
- `scripts/ci/check-architecture-sync.py` запускается на каждом pull request и
  ...
  `unica.*` в реестре Rust, не тронув ни одного нормативного документа: записи
  решения `spec/decisions/NNNN-*.md`, `spec/architecture/invariants.md`,
  `spec/architecture/quality-requirements.md` или плана приёмки в
  `spec/acceptance/`. Ни глоссарий, ни список рисков, ни индекс каталога
  доказательством не считаются: правку запятой в глоссарии нельзя выдать за
  описание нового инструмента. Скрипт смотрит только на объявления
  инструментов, поэтому изменение поведения существующего инструмента ловит
  ревью, а не он; и он не проверяет, что изменённый документ действительно
  владеет этим инструментом, — это тоже работа ревью.

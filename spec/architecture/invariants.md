...
- **Rule:** Публичная группа метаданных содержит только `unica.meta.info`,
  ...
  регистрируются и не принимаются. Публикуемая схема `operations` описывает
  условно полные закрытые варианты каждой операции: у варианта есть собственное
  значение `op`, точный набор обязательных полей и запрет неизвестных полей.
  Для `meta.add` вид владельца берётся из `kind`, а для `meta.edit` — из
  канонизированного корня `metadataPath`; этот вид ограничивает свойства и их
  перечисления, коллекции, область `scope` и допустимые поля элемента теми же
  закрытыми реестрами, которые использует доменное преобразование.
...
- **Rule:** Сетевой выход поставщиков документации задаётся файлом
  ...
  единой закрытой схемы с полями `version`, `operational`, `network` и
  `providers`: общий разборщик до потребительского разбора отвергает
  синтаксическую ошибку, неизвестное корневое поле и недопустимую
  присутствующую `version`. Слой только с `network` и `providers` сохраняет
  совместимость без `version`, но слой с `[operational]` обязан содержать
  `version = 1`; `version` другого слоя этого требования не удовлетворяет,
  а ошибка общего корня отказывает всем его потребителям. Политика разбирает только `network` и
...
- **Rule:** Для `unica.code.search`, `unica.code.definition`,
  ...
  самостоятельные присутствующие слои той же закрытой схемы `version`,
  `operational`, `network`, `providers`, что и сетевая политика: прежний слой
  только с `network` и `providers` допустим без `version`, но слой с
  `[operational]` обязан содержать `version = 1`, и `version` другого слоя не
  заменяет это требование; недопустимый общий корень отказывает всем его
  потребителям. Операционный
  потребитель проверяет только `[operational]`: ошибка `network` или

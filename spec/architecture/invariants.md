...
- **Rule:** `unica.code.search` принимает ровно один селектор: канонический
  ...
  и точность
  счёта; `empty` означает только доказанный точный ноль, а `limitReached` и
  `timedOut` сохраняют неполный префикс. Попадание содержит логическое
...
- **Rule:** Для нормализованной пары рабочего пространства и корня исходников
  ...
  доказанный событийный барьер и не обходит дерево; холодный старт, изменение и
  потеря доверия выполняют полное согласование только там, где следующий барьер
  может доказать непрерывность. Платформа или файловая система без доказуемого
  барьера оставляет ревизию недоверенной, а семантический поиск недоступным;
  повторные обходы не служат доказательством свежести. Маркер готовности индекса
  хранит полную ревизию,

...
### INV-MCP-RUNTIME-RECEIPT — Применённый runtime принадлежит долговременной записи

- **Rule:** Применённая операция `unica.runtime.execute` заводит долговременную
  запись задания, публикует `notifications/progress` по смене её фазы и отметке
  `heartbeat_at_ms` и возвращает терминальный результат этой записи в исходном
  `tools/call`; отмена вне небезопасной фазы уничтожает всю группу процессов, в
  небезопасной фазе вызов прекращает ожидание и отвечает квитанцией с `jobId` и
  названной причиной, а обрыв вызова хостом задание не отменяет. Предпросмотр
  остаётся синхронным и без побочных эффектов, неклассифицированная операция
  по-прежнему отказывает, а унаследованные `unica.build.*` запасным путём не
  предлагаются, а ограниченный `launch` с `waitForExit: true` остаётся
  синхронным в исходном вызове, потому что ограничен собственным
  `waitTimeoutMs`.
- **Decision:** ADR-0074
...
- **Rule:** Для нормализованной пары рабочего пространства и корня исходников
  ...
  недоступным; повторные обходы не служат доказательством свежести. Барьер
  отсекает то же, что и обход корпуса: служебный каталог `.build` и корень
  кеша, лежащий строго внутри корня исходников, — кеши самой Unica лежат
  внутри корня исходников, и барьер, сообщающий об изменениях, которых нет в
  карте корпуса, никогда не сходится; путь-скоупированный пробел наблюдателя,
  чьё поддерево не достигает неотсечённого корпуса, доверие не роняет. Маркер

...
- **Rule:** Публичная группа метаданных содержит только `unica.meta.info`,
  ...
  регистрируются и не принимаются. По ADR-0025 публикуемая inline-схема
  `operations` описывает закрытые формы пяти операций и остаётся независимым от
  владельца надмножеством без `allOf`/`if`/`then`: она запрещает неизвестные
  поля и неверные типы, но не является вторым арбитром сочетания вида объекта с
  операцией. Для `meta.add` вид владельца берётся из `kind`, а для `meta.edit` —
...
- **Rule:** Сетевой выход поставщиков документации задаётся файлом
  ...
  перекрывающим по-ключево. Каждый присутствующий файл — самостоятельный слой
  единой неверсионируемой закрытой схемы с полями `operational`, `network` и
  `providers`: общий разборщик до потребительского разбора отвергает
  синтаксическую ошибку и неизвестное корневое поле, включая `version`, а
  ошибка общего корня отказывает всем его потребителям. Политика разбирает
  только `network` и
  `providers` по правилу «неясность — отказ»: ошибка `operational` её не
  затрагивает, а неизвестная секция, ключ, идентификатор поставщика или
  значение её поддеревьев — жёсткий отказ вызова, не молчаливое разрешение.
  Запрещённый поставщик отвечает секцией `unavailable` с причиной
...
- **Rule:** Общая предпроверка `unica.runtime.execute` и
  ...
  вызов в один `fullRebuild`, а нечитаемое или противоречивое свидетельство либо
  неизвестный селектор, недоказанный или противоречащий глобальному режиму
  физический формат либо неосновной файл конфигурации завершаются до запуска
  объяснимым отказом с рекомендацией явной полной пересборки. Селектор и
  глобальный режим разрешаются по тем же правилам, что в закреплённом
  `v8-runner`: значение формата допускает только точные `DESIGNER` и `EDT`, а
  отсутствие означает режим Конфигуратора. Явный
  `fullRebuild` обходит чтение поддержки. Проверенный `config` материализуется
  как абсолютный канонический путь к основному `v8project.yaml`, не позволяя
  `V8TR_CONFIG`, перенаправлению символьной ссылки либо подмене относительного
  рабочего каталога заменить проверенный проект. Разрешение инкрементального
  режима связывается с SHA-256 файла конфигурации, корнем и эпохой рабочего
  пространства и повторно проверяется после нового обнаружения рабочего
  пространства перед фактическим запуском, включая фоновый обработчик. Ответ
  предметного читателя, разрешающий инкрементальный режим, связывается с
  непрозрачным снимком точных байтов свидетельства поставщика; снимок повторно
  сверяется
  после ответа и после заключительного обнаружения рабочего пространства, а
  точные входы карты проекта, влияющие на разрешение выбранных конфигураций,
  связывают его с тем же корнем набора. Размеры файловых снимков карты и
  состояния поддержки ограничены; превышение завершает предпроверку отказом до
  разбора.
  Захват фонового перехода
  `Queued` → `Running` сериализован с отменой и восстановлением протухшей записи;
  отмена до захвата не запускает процесс и не оставляет активного задания, а
  терминальный `Lost` не перезаписывается активацией. Предпроверка не читает
  частное состояние `v8-runner`.
- **Decision:** ADR-0059, ADR-0054, ADR-0003
...
- **Rule:** Разрешение цели выполняется под явной политикой вида: политика
  решает каждый вид цели, включая корень набора исходников без `metadataPath`,
  пишущая операция запрашивает только терминал модуля и отклоняет любой другой
  вид стабильным `TargetKindMismatch`, закрытая ручка несёт вид, под которым
  выдана, и повторная проверка выполняется под той же политикой, поэтому
  расширение резолвера на новый вид цели не расширяет право записи.
- **Decision:** ADR-0021
- **Check:** `ci-test` — `crates/unica-coder/src/infrastructure/platform_xml_source_targets.rs`

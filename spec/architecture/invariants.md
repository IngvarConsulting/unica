...
- **Rule:** Для `unica.code.search`, `unica.code.definition`,
  ...
  готовности и операция `RLM` с бюджетом вызывающей стороны получают полный
  остаток срока вызова без 120-секундной верхней границы; внутренний протокол с
  `schema_version = 4` без потерь переносит его полями `timeout_seconds` и
  `timeout_nanos`, а запись сервиса с `schema_version = 3` не переиспользуется и
  заменяется сервисом текущей схемы. Отмена имеет приоритет над одновременно
  полученной ошибкой загрузки; остальные

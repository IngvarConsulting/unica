...
- **Rule:** Рабочее пространство без `v8project.yaml` получает наборы исходников
  ...
  вместе со списком маркеров корня: перечисляя каталог, автообнаружение
  публикует лишь настоящий подкаталог, открытый без перехода по ссылке, а
  постороннюю запись пропускает вместо обрыва; набор называется именем

...
- **Rule:** Сетевой выход поставщиков документации задаётся файлом
  ...
  перекрывающим по-ключево. Оба файла образуют один закрытый корень с полями
  `version`, `operational`, `network` и `providers`: общий разборщик до
  потребительского разбора отвергает синтаксическую ошибку, неизвестное
  корневое поле и недопустимую присутствующую `version`. Слой только с
  `network` и `providers` сохраняет совместимость без `version`, но слой с
  `[operational]` обязан содержать `version = 1`; ошибка общего корня
  отказывает всем его потребителям. Политика разбирает только `network` и
...
- **Rule:** Для `unica.code.search`, `unica.code.definition`,
  ...
  одного рабочего пространства не переиспользуется другим. Снимок читает тот
  же закрытый корень `version`, `operational`, `network`, `providers`, что и
  сетевая политика: прежний слой только с `network` и `providers` допустим без
  `version`, но слой с `[operational]` обязан содержать `version = 1`, а
  недопустимый общий корень отказывает всем его потребителям. Операционный

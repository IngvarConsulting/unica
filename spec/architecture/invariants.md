...
- **Rule:** `unica.documentation.search` возвращает по одной секции на
  поставщика в порядке реестра. Каждое попадание несёт поставщика, корпус,
  смысл источника, авторитетность и применимую версию. Ранги и оценки
  локальны для секции: Unica не объединяет, не пересортировывает и не удаляет
  совпадения между секциями. Отказ одного поставщика остаётся в его секции.
- **Decision:** ADR-0029
...
- **Rule:** Поставщик справки платформы читает контейнеры установки и держит
  карту корпусов в памяти процесса, привязанной к версии платформы. Индекс,
  кеш и распакованные страницы на диск не пишутся, в рабочее дерево не
  попадают и в пакет не включаются.
- **Decision:** ADR-0029

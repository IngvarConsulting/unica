...
### INV-SOURCE-ADAPTER-PIPELINE — Чтение проходит явную цепочку выбора адаптера

- **Rule:** Любое семейство источника читается через `SourceProvider`,
  `FormatProbe`, `AdapterRegistry`, `VersionedDecoder`, `NativeSnapshot`,
  `SemanticProjector`, `SemanticSnapshot` и `CapabilityEngine`; неизвестная,
  неоднозначная или будущая версия отклоняется, а ближайшая известная версия
  никогда не выбирается.
- **Decision:** ADR-0019
- **Check:** `ci-test` — `crates/unica-adapter-platform-xml/tests/task7_fix_round3_architecture.rs`
- **Scope:** runtime, source

### INV-SOURCE-FAMILY-PRIVATE-NATIVE — Нативный формат принадлежит семейному крейту

- **Rule:** Семейный крейт владеет layout, schema, parser, serializer, writer и
  приватной диспетчеризацией версий, наружу отдаёт только factory и нейтральные
  core-порты, а Platform XML, EDT, CF и FileDB не выносят нативные имена, пути,
  страницы, offsets, container layout или table format в application и
  `unica-coder`.
- **Decision:** ADR-0019
- **Check:** `ci-test` — `crates/unica-adapter-platform-xml/tests/task7_fix_round3_architecture.rs`
- **Check:** `ci-test` — `crates/unica-adapter-platform-xml/tests/task8_writer_architecture.rs`
- **Scope:** source

### INV-SOURCE-OPAQUE-REVISION-SESSION — Ревизия источника неизменяема и непрозрачна

- **Rule:** Открытая сессия источника привязана к одной неизменяемой ревизии,
  лениво материализует только запрошенные данные и не раскрывает физические
  paths, offsets, handles, parser nodes или нативные object keys за границей
  семейного адаптера.
- **Decision:** ADR-0019
- **Check:** `ci-test` — `crates/unica-adapter-platform-xml/tests/task7_fix_round3_architecture.rs`
- **Scope:** runtime, source

### INV-SOURCE-CLOSED-SEMANTIC-VOCABULARY — Семантический словарь закрыт

- **Rule:** ID объектов, свойств, отношений и facets, типы и состояния значений
  принадлежат `unica-format-core`; произвольный нативный extension key
  недопустим, а неизвестный значимый факт даёт `partial` и нейтральную
  диагностику вместо молчаливой потери.
- **Decision:** ADR-0019
- **Check:** `ci-test` — `crates/unica-adapter-platform-xml/tests/legacy_parity.rs`
- **Check:** `ci-test` — `crates/unica-adapter-platform-xml/tests/task7_fix_round3_architecture.rs`
- **Scope:** runtime, source

### INV-SOURCE-AUTHENTICATED-CURSOR — Continuation непрозрачен и привязан к снимку

- **Rule:** AI-facing request и response используют только типизированный JSON,
  а continuation передаётся непрозрачным аутентифицированным base64url-cursor,
  привязанным к source, revision, authorization, target, relation, selection и
  position; legacy text и offset-pagination не являются совместимым путём.
- **Decision:** ADR-0019
- **Check:** `ci-test` — `crates/unica-format-core/src/navigation.rs`
- **Scope:** runtime, source

### INV-SOURCE-TYPED-WRITER-BOUNDARY — Writer отделён от reader и нативного payload

- **Rule:** Writer получает закрытую неизменяемую семейно-нейтральную команду и
  возвращает закрытый типизированный lifecycle/result; перед первой записью он
  повторно проверяет revision, capability, coverage и authorability, а host
  только отображает MCP-параметры и результат и не собирает нативный payload.
- **Decision:** ADR-0019
- **Check:** `ci-test` — `crates/unica-adapter-platform-xml/tests/task8_writer_architecture.rs`
- **Check:** `ci-test` — `crates/unica-adapter-platform-xml/tests/task8_fix_round1_preservation_matrix.rs`
- **Scope:** runtime, source
...
- **Decision:** ADR-0016, ADR-0019
- **Check:** `ci-test` — `crates/unica-adapter-platform-xml/tests/task8_fix_round4_contract.rs`
...
- **Decision:** ADR-0016, ADR-0019
- **Check:** `ci-test` — `crates/unica-adapter-platform-xml/src/versions/v2_20/writers/compile_transaction.rs`
- **Check:** `ci-test` — `crates/unica-adapter-platform-xml/tests/task8_writer_ports.rs`
- **Scope:** runtime

...
- [ ] Чтение источника проходит зарегистрированную versioned-adapter цепочку
  без выбора ближайшей версии (`INV-SOURCE-ADAPTER-PIPELINE`).
- [ ] Layout, schema, parser, writer и версия остаются внутри семейного крейта,
  а application/coder видят только нейтральные порты
  (`INV-SOURCE-FAMILY-PRIVATE-NATIVE`).
- [ ] Сессии ревизий остаются неизменяемыми и непрозрачными, а закрытый
  семантический словарь не принимает нативные extension-ключи
  (`INV-SOURCE-OPAQUE-REVISION-SESSION`,
  `INV-SOURCE-CLOSED-SEMANTIC-VOCABULARY`).
- [ ] AI-контракт остаётся JSON-only, а continuation передаётся только
  аутентифицированным непрозрачным cursor
  (`INV-SOURCE-AUTHENTICATED-CURSOR`).
- [ ] Writer принимает закрытую семантическую команду, повторно проверяет
  capability/authorability и публикует через общий lifecycle
  (`INV-SOURCE-TYPED-WRITER-BOUNDARY`,
  `REQ-SAFETY-WRITER-REVALIDATION`).
...
- [ ] Индексы остались синхронными: [`../decisions/README.md`](../decisions/README.md)
  ...
  каталога `spec/architecture/` (`INV-DOC-INDEX-SYNC`).
...
индексах: индекс решений
перечисляет ровно записи на диске, а `spec/README.md` — каждый документ каталога
`spec/architecture/` (`INV-DOC-INDEX-SYNC`); каждая неручная проверка записи называет

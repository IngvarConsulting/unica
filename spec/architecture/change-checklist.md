...
- [ ] Изменённый путь `unica.runtime.execute` сохраняет терминальный результат
  исходного вызова, а неизвестная или недоказанная применённая ветвь отказывает
  до обнаружения рабочего пространства и запуска процесса
...
- [ ] `unica.code.search`, `unica.code.definition`, `unica.code.outline` и
  `unica.code.diagnostics` с `action=analyze` разрешают после рабочего
  пространства ровно один снимок
  `OperationalConfig`; остальные вызовы не разрешают `OperationalConfig` и не
  читают `[operational]`. Потребители сетевой политики документации и стандартов
  независимо читают `network` и `providers` из тех же файлов до сетевого
  обращения (`INV-APP-CONFIG-SNAPSHOT`,
...
- [ ] Оркестрация диагностик сохраняет порядок и происхождение поставщиков,
  локализует их отказы и не дедуплицирует наблюдения между ними
...
- [ ] Каждая новая применённая ветвь runtime явно классифицирована по
  возможности безопасного завершения и не допускается одним предположением о
  длительности (`INV-MCP-RUNTIME-RECEIPT`).
...
- [ ] Миграция читателя явно сохраняет `bridge` либо принимает отдельное
  решение о `directSwitch`; прямое переключение `unica.code.diagnostics` одним
  изменением обновляет публичную схему, обработчик, скилл, исполняемые примеры,
  ведомость поверхности, руководство по миграции, проверку релиза и исполняемые
  тесты
...
- [ ] Запись `.mcp.json` публичного пакета сохраняет собственный бюджет старта,
  покрывающий холодную установку ядра (`REQ-REL-COLD-INSTALL-BUDGET`).
- [ ] Порядок разрешения кеша runtime не изменился (`INV-CACHE-RUNTIME-ROOT-ORDER`).
- [ ] Идентичность кеша включает SHA-256 доставляемого asset, а `prefetch`
  закрывает полный набор артефактов цели (`INV-PKG-VERIFIED-ATOMIC-INSTALL`,
  `REQ-REL-INSTALL-ONCE`).
- [ ] Параллельные followers одной доставки немедленно освобождают admission
  slot и возвращают типизированное состояние (`INV-MCP-DELIVERY-STATE`,
...
`tests/ci/test_architecture_registry.py` проверяет, что каждый процитированный
здесь ID существует в реестре, и отдельно удерживает разделение операционного
снимка и сетевой политики: documentation/standards не читают `[operational]`, но
до сетевого обращения читают `network` и `providers` из тех же файлов. Смысл
остальных пунктов остаётся на ревью. Остальные названные здесь правила тест
удерживает на самих реестрах и индексах: индекс решений перечисляет ровно записи
на диске, а `spec/README.md` —

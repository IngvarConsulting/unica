...
- `code_intelligence` — `CodeSearchCoordinator` и общий исполнитель операций
  ...
  (INV-MCP-CODE-SEARCH-ROLES, INV-MCP-BOUNDED-ADMISSION).
...
- `workspace_services` — `WorkspaceServiceManager` и
  ...
  пространства, отдельные тёплые транспорты `bsl-analyzer` и `rlm-bsl-mcp`,
  логическая сессия RLM и внутренний протокол JSONL с `schema_version = 4`.
  Обычные и управляющие пути сохраняют свои умолчания, а рабочие запросы
  анализатора и RLM с бюджетом вызывающей стороны передают остаток без потерь
  полями
  `timeout_seconds` и `timeout_nanos` и не ограничивают его 120 секундами
  В том же сервисе живёт один `SourceRevisionService` для RLM
  (ADR-0018, ADR-0040, ADR-0057).
- `source_revision` — доверенная ревизия корпуса RLM: сохранённое поколение,
  SHA-256, согласование после потери доверия и платформенный fence
  (INV-CACHE-RLM-REVISION).
- `workspace_index` — жизненный цикл постоянного индекса BSL, который строит
  поставляемый `rlm-bsl-index`: холодное поколение построителя 15 в
  `rlm-bsl/index-v15`, отдельные маркеры состояния и блокировки поколения,
  запрет чтения при `building` и `incomplete`, сохранение и игнорирование
  построителя 14, классификация готовности, сердцебиение, фоновая сборка и
  обновление, а также атомарная публикация готовности только для захваченной
  полной ревизии
  (INV-CACHE-GENERATION-CUTOVER).

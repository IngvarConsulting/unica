...
пространство через порт, проверяет контекст вызова, оценивает страж формата и
пропускает вызов через стражи маршрута runtime и синхронизации выгрузки. Для
мутирующего инструмента он оценивает страж поддержки. Перед обработчиком четыре
затронутые операции с кодом разрешают один `OperationalConfig`; остальные
вызовы не обращаются к его файловым слоям. Затем приложение вызывает
обработчик, собирает доменные события и отчитывается о влиянии на кеш. Страж
...
- `workspace_services` — `WorkspaceServiceManager` и
  ...
  управляющие пути сохраняют свои умолчания, а RLM с бюджетом вызывающей
  стороны передаёт остаток без потерь полями `timeout_seconds` и
  `timeout_nanos` и не ограничивает его 120 секундами
  (ADR-0018, ADR-0039).

...
### REQ-SAFETY-WRITER-REVALIDATION — Запись повторно доказывает право на мутацию

- **Rule:** Между планированием и публикацией writer повторно проверяет
  ожидаемую revision, preimages, capability, coverage и authorability; отмена
  или изменение любого доказательства даёт типизированный отказ либо rollback,
  а не частичную или разрешённую запись.
- **Decision:** ADR-0019
...
- **Check:** `ci-test` — `crates/unica-adapter-platform-xml/src/versions/v2_20/writers/single_file_publisher.rs`
- **Scope:** runtime
...
- **Rule:** Семейный адаптер Platform XML выбирает действующую линейку платформы
  и формат выгрузки из своего приватного version profile, а каждое намеренное
  отклонение от официального отображения платформы записано в матрицу
  отклонений; application и `unica-coder` не содержат native version literal.
...
### REQ-COMPAT-SEMANTIC-COMPLETENESS — Нейтральная модель не беднее полезного legacy-чтения

- **Rule:** Для заявленного coverage семантический результат сохраняет как
  минимум полезные факты legacy full/drill-down, включая rights, полные type
  descriptions, children, relations, facets и bounded opaque-content evidence;
  неизвестный значимый факт понижает результат до `partial`.
- **Decision:** ADR-0019
...
### REQ-REL-NO-SILENT-STALL — Застрявший релиз докладывает, а не догадывается

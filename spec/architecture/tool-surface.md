...
**Результат сейчас:** `data`: provider-neutral результат с `sourceSet`/`metadataPath`, секциями `providers`, логическими `location`, внутренним `focus` и плоскими элементами; переходная неготовность использует общий с поиском `dependencyPending` и сохраняет подсказку поставщика о повторе (ADR-0077); физические пути и формат одного анализатора наружу не выходят (отвечают типизированным `data`)

...
- Инструментов: **71**
- Отвечают типизированным `data`: **40**
- Типизированы частично: часть результата всё ещё текст: **2**
- Отвечают снимком задания в `job`: **6**
- Отвечают прозой в `stdout`: **23**
...
- Осталось перевести на типизированный `data` в границах работы: **3**
...
- Выполнить выгрузка набора исходников через единый MCP без ручной командной строки
...
- Выполнить загрузка исходников в базу через единый MCP без ручной командной строки
- Проверить предпросмотром, что будет запущено, до фактического запуска
...
- Выполнить сборка CF/CFE через единый MCP без ручной командной строки
- Проверить предпросмотром, что будет запущено, до фактического запуска
...
**Целевой контракт:** `data`: один ответ со всеми секциями — наборы данных с полями, связи, вычисляемые поля, ресурсы, параметры, варианты настроек, макеты, запросы. Одиннадцать режимов `Mode` становятся проекцией над ним, а не одиннадцатью форматами текста; `Limit`/`Offset` уходят вместе с построчной вёрсткой
...
**Целевой контракт:** `data`: адрес, вид, имя, синоним, поддержка, `properties` — свойства **именами платформы** (NumberType, Hierarchical, LevelCount…) вместо переведённой прозы, владельцы, реквизиты/измерения/ресурсы, ТЧ с колонками, значения перечисления, формы, макеты, команды. `Mode` (overview/brief/full) и `Name` становятся проекцией над одним ответом, как в subsystem.info

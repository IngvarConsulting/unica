...
**Результат сейчас:** `data`: provider-neutral результат с `sourceSet`/`metadataPath`, секциями `providers`, логическими `location`, внутренним `focus` и плоскими элементами; переходная неготовность использует общий с поиском `dependencyPending`, сохраняет подсказку поставщика о повторе и публикует верхний `state=pending` при `ok=false` (ADR-0077); физические пути и формат одного анализатора наружу не выходят (отвечают типизированным `data`)
...
**Результат сейчас:** `data`: три ролевые секции с логическими местоположениями, полнотой, машинной причиной завершения и retryability, ранжированием, счётом и происхождением; `dependencyPending` сохраняет доступные `retryAfterMs` и `state` поставщика по общему с диагностикой словарю (ADR-0077), а ход приходит через typed MCP progress (ADR-0056) (отвечают типизированным `data`)

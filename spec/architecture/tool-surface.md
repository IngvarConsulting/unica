...
**Результат сейчас:** типизированный `data` с побайтовым diff, проверкой BSL и `validation.methodNaming` по `std647`: точные `automatedChecks` с кодом и связанным пунктом стандарта, диагностики и обязательный `semanticReview` каждого нового или переименованного метода (отвечают типизированным `data`)

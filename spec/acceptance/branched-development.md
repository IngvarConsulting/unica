...
| BD-07 | Every scalar/module/add-add/delete-modify/reference/support conflict is typed and explicitly resolved through its persisted non-empty canonical `allowedResolutions`; an unavailable pair is rejected before any receipt/decision mutation, and no release infers a static kind matrix that is not backed by the platform classifier | Settings/parser fixtures covering every emitted kind/resolution pair and every rejected pair, decision journal, sandbox replay and real platform cases |
...
| BD-14 | MCP/worker/process interruption resumes from durable state at every remote-effect barrier; `observed` is an fsynced barrier, never an operation state, and a lost terminal response reconstructs typed gate-history, separate-IB lease, support-recovery finalization, and post-merge commit evidence from status/receipts and a digest-bound exact recovery plan. A state-root override cannot hide an unresolved task/start attempt, and persistent target/account reservations survive lease loss and response loss. `readOnly` is not a durable operation policy; a stored/legacy `OperationRecord` claiming it fails closed as `stateCorrupt` before status/replay dispatch and cannot cause a lease, worker, receipt, migration coercion, replay, or effect | Unit crash matrix with no `state: observed`; response-loss-after-observed-barrier tests; pure storage-schema and fake replay negatives for a persisted `policy: readOnly` record proving deterministic `stateCorrupt` and zero CAS/worker/receipt/effect calls; two independent two-host races, first same target with different normalized accounts and then different targets with the same normalized account, each with exactly one admitted start and the key-specific loser code; recovery-plan/receipt approval tests, cross-root locator tests, fake worker death, and real dangerous-phase restart scenarios |
...
| BD-22 | Every tool obeys the exact generated request/result schema, per-variant mutability/preview policy, closed completed/stopped/rejected union, typed data/evidence, and operation replay rules. JSON-derived digests use RFC 8785 JCS only, operation inputs are domain-separated by exact tool name and execution policy, and durable operation/input/status-projection schemas exclude `readOnly` | Committed generated JSON Schema/variant-policy/storage snapshots plus application and packaged MCP contract tests, including evidence-preserving domain stops, data-free safety rejections, canonical-JSON golden vectors, different-tool/different-policy replay negatives, `ActiveOperationStatus.policy` read-only schema rejection, and load/replay rejection of a persisted read-only operation record before any effect port is called |
...
| BD-24 | File and client/server originals plus file/server repository transports are enabled only by their own topology evidence. Each endpoint declares `hostConfined` or `multiHost`; any multi-host topology requires retained proof of a reachable linearizable coordinator atomically excluding target and repository-account starts | Separate disposable real-fixture rows; an isolated same-target/different-normalized-account two-host race with exactly one winner and `targetReservationBusy` loser; a separate different-target/same-normalized-account race with exactly one winner and `repositoryAccountReservationBusy` loser; response-loss receipt observation, cross-topology and missing-coordinator negative preflight tests |
...
| `unica.merge.conflicts` | None | All seven typed conflict kinds, three-side hashes, and the exact persisted non-empty canonical per-conflict allowed-resolution list; handler registration is gated on a fixture-backed classifier rather than enum-name inference |
| `unica.merge.resolve` | Decision journal | Typed payload, rationale, owned/fingerprinted manual data, and persisted-list membership checked before receipt consumption or journal mutation |
...
| `unica.repository.status` | None | Binding, main/repository/database sync, journal/last-observed conflicts with completeness, explicit typed `target` plus presentation-only `targetDisplay`, nullable unproven owner fields; display never participates in identity/control flow and no global-live-lock or reconciliation claim is made |
...
- atomic journal/schema migration and every write barrier, with exactly
  ...
  states and only `DurableExecutionPolicy` in records/input/status projections;
  a persisted `readOnly` record is rejected as `stateCorrupt` before replay or
  migration writes, while crash after the fsynced observed barrier reconstructs
  terminal output without replay;
- stable project/target/account identities, collision/relocation rules,
  ...
  isolated two-host same-target/different-account and
  different-target/same-account contention with the key-specific loser codes,
  persistent fenced
...
recovery state. A separate pure storage/fake-replay case injects a validly
encoded legacy `OperationRecord` with `policy: readOnly`, proves deterministic
`stateCorrupt` before status/replay classification, and asserts zero lease CAS,
worker, receipt, dispatch, migration-write, and effect-port calls. Repository
status schema snapshots also reject `policy: readOnly` in
`ActiveOperationStatus` because it can project only a durable record. Repository
status schema fixtures require the literal `targetDisplay` field beside typed
`target`, reject its omission/renaming, and prove changing display alone cannot
change conflict identity, ordering, or control flow. One generic partial-effect
test cannot satisfy several. The suite verifies domain behavior but cannot
create a capability manifest row.

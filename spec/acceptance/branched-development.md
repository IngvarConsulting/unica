...
| BD-14 | MCP/worker/process interruption resumes from durable state at every remote-effect barrier; a lost terminal response reconstructs typed gate-history, separate-IB lease, support-recovery finalization, and post-merge commit evidence from status/receipts and a digest-bound exact recovery plan; a state-root override cannot hide an unresolved task or start attempt | Unit crash matrix, response-loss-after-observed tests for every handle-producing phase/disposition, recovery-plan/receipt approval tests, cross-root locator tests, fake worker death, and real dangerous-phase restart scenarios |
...
| --- | --- | --- |
| `unica.branched.start` | Durable journal and owned work root | Valid deny-unknown profile, task ID, operation ID, exact platform row plus separate retention-provider row, target/account leases, exact per-layer immutable CF source/provider-object mapping and WORM-retention entries, pairwise-disjoint protected/provider roots, and both-mode conditional inspection/exclusive-configuration-lease endpoint capability; reject missing/stale/mismatched rows, skipped cases, and digest/evidence mismatch before task creation |
...
| `unica.repository.lock` | Repository locks | Approved exact plan, root guard first plus under-lock gate recheck, then per-object journal under the dedicated user and compensation proof; conflict returns object, nullable owner, redacted diagnostic and external action |

...
Схема для вычитки, 2026-09-17; исследован checkout `9b286de5`.
Она показывает, **когда агент узнаёт ограничение и что вызывает следующее чтение**.
Целевая схема подключена частично: новый `arch/rules/` и редактура через
`unica-writing` уже доступны. Основной порядок разработки пока остаётся
в AGENTS; этот обзор сам по себе не меняет инструкции или CI.
...
flowchart TD
    ...
    CL -->|"сейчас: агент читает ссылку; цель: импорт"| A
    ...
    Q -.->|"работа над текстом: уже подключено"| W["Прочитать unica-writing"]
    META --> Q
    A -.->|"нет метаданных общих скиллов: целевой маршрут Claude"| FM["Прочитать только шапки .agents/skills/*/SKILL.md"]
    FM --> Q
    Q -->|"разработка Unica: целевой маршрут"| D["Прочитать unica-development"]
...
Claude Code загружает CLAUDE.md; **сейчас** в нём обычная ссылка на AGENTS,
по которой агент должен прочитать файл. **Предлагается** заменить этот переход
импортом `@AGENTS.md`, чтобы общий вход загружался хостом.
...
Ниже целевой маршрут. `unica-development` и `unica-testing` пока не созданы.
Сейчас их обязанности распределены между AGENTS, CONTRIBUTING и runner.

```mermaid
flowchart TD
    T["Задача и unica-development"] --> B["Класс задачи, затронутые компоненты и поведение"]
    ...
    EXTRA --> K
    SELECT -->|"дефект"| RED["Воспроизвести падение по его причине"]
    RED --> CODE["Изменить код и содержательные тесты"]
    SELECT -->|"новая возможность или иная правка"| CODE
...
Независимость скептика и reviewer определяется действующим AGENTS; отдельный
...
| Нужна история конкретного решения | Одна найденная историческая запись | Основание прошлого выбора; архив не задаёт действующее правило |
...
3. Найти `путь::имя_теста` в действующем `arch/`. Открыть совпадения в поле
   `check` и прочитать применимые правила. Для нынешнего реестра учитывать
   статус записи; исторические записи не подменяют действующие.
...
```sh
rg -n -F 'crates/unica-coder/src/infrastructure/workspace_actor.rs::capabilities_do_not_cross_distinct_actor_instances_with_equal_identity' arch/
```

Он находит несколько правил. Агент читает их применимую часть; связь не обязана
...
Общий целевой каталог — `.agents/skills/<name>/SKILL.md`. В Codex он
обнаруживается штатно. После удаления `.claude/` Claude по указанию общего
AGENTS читает шапки `name`/`description` из этого каталога, выбирает скилл
и открывает его тело. Если каталог уже передан хостом, повторять чтение шапок
не нужно. Нативное обнаружение Claude по этому пути не предполагается.
Продуктовые навыки
`plugins/unica/skills/` относятся к работе с 1С через Unica: при разработке
их поведения читается только затронутый скилл.

Уже создан [unica-writing](.agents/skills/unica-writing/SKILL.md).
AGENTS направляет к нему при создании, редактуре или ревью текста. Его
действующий `description`:
...
Тело читается только при работе с текстом; устанавливать внешний `/post`
для этого не нужно. Полный маршрут обнаружения навыков обоими хостами
остаётся предметом проверки этапов 4–5.

У остальных будущих файлов пока нет шапок. Ниже **предлагаемые значения `description`**,
а не утверждение об уже установленных навыках:

| Имя | Предлагаемый `description` | Когда читается тело |
| --- | --- | --- |
| `unica-development` | Разработка самого Unica: исследование задачи, выбор применимых правил продукта, изменение кода и подготовка PR. Используй для работы в репозитории Unica. | В начале задачи разработки |
| `unica-testing` | Тестирование Unica: выбор проверяемого поведения и места теста, small/medium/large, разметка, написание тестов и разбор падений. Используй при выборе, изменении, запуске или ревью проверок. | Перед выбором проверок; при новом вопросе о тестировании |
| `unica-release` | Выпуск версии Unica в публичный маркетплейс или продолжение незавершённого выпуска. Используй при запросе выпустить, продвинуть или завершить релиз. | Только для выпуска |
| `unica-architecture` — кандидат | Продуктовые обязательства Unica: предложить новое правило или изменение существующего, сравнить альтернативы и связать правило с содержательной проверкой. | Если нужен отдельный маршрут принятия правила |
| `unica-review` — кандидат | Скептическая оценка замысла или проверка diff Unica: найти нарушения, лишнюю сложность и пробелы проверок, показать основания и последствия. | При оценке решения или ревью |

Кандидаты не становятся обязательными только из-за этой таблицы.
Основной скилл направляет в testing/release и не копирует их порядок.

До переноса в общий каталог выпуск по-прежнему описывает
[release](.claude/skills/release/SKILL.md). Его действующий `description`:
...
AGENTS также упоминает `brainstorming` и `writing-plans`. В текущей сессии
...
Широкий триггер brainstorming пересекается с будущим unica-development.
...
Это целевое распределение; реализованы пилот правил и маршрут редактуры.
...
| `.agents/skills/unica-development/SKILL.md` | Выбрать область и порядок работы, нужные знания и проверки | При разработке Unica |
...
| `.agents/skills/unica-writing/SKILL.md` | Ясные тексты разных жанров с сохранением фактов и границ гарантий | При работе над текстом; уже подключён из AGENTS |
| `.agents/skills/unica-release/SKILL.md` | Определить этап выпуска и направить в runbook | При выпуске |
| `arch/*.md` и вложенные записи | Согласованные атомарные гарантии продукта с `check` | По области задачи и по упавшему тесту |
...
До подключения целевой схемы остаются видимые расхождения:

- Основной `unica-development` и `unica-testing` ещё не созданы; общие
  MCP-навыки уже выбираются по необходимости.
- Десять правил в `arch/rules/` ищутся по области и тестам; формулировки
  пилота ещё ожидают вычитки владельцем.
- Из новых скиллов подключён `unica-writing`; release ещё лежит в `.claude/`.
- Ссылка из CLAUDE на AGENTS требует чтения агентом; целевой импорт ещё не сделан.
- Загрузка правила после ошибки требует инструкции в unica-testing;
  сами runner и Markdown сейчас этот переход не выполняют.

Схему проверяем наблюдением за новой сессией: какие источники действительно
попали в контекст, какое правило прочитано до решения, какой тест исполнился
и что агент дочитал после ошибки. Для этого подходят локальная правка,
MCP-изменение и падение теста с `check`. Заявления «прочитал всё» и тесты
на наличие слов в этом файле не проверяют работу маршрута.

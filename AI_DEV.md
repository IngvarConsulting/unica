...
Целевая схема ещё не подключена: этот файл не меняет AGENTS, скиллы или CI.
План миграции уточняется после её обсуждения.
...
У будущих файлов пока нет шапок. Ниже **предлагаемые значения `description`**,
...
Сейчас единственный репозиторный dev-скилл —
...
Это предложение для последующего уточнения плана; файлы пока не перестроены.
...
- AGENTS и CONTRIBUTING всё ещё требуют общий MCP-комплект; пользователь
  уже выбрал основной unica-development и общие навыки по необходимости.
- Входы направляют через `arch/index.md`; новый пилот должен находить нужные
  записи без сводного реестра, по области, исходникам и связанным тестам.
- Будущие скиллы отсутствуют; release ещё лежит в `.claude/`.

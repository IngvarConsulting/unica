...
_EXACT_SECRET_KEYS = frozenset({"connection", "pwd"})
...
def _validate_report_path(
    ...
    *,
    ...
    if _is_relative_to(target, database) or _is_relative_to(target, sources):
        raise SourceError("report path must be outside both input trees")
    return target
...
def _validate_evidence_directory(
    ...
    *,
    ...
    if (
        ...
        or _is_relative_to(report_path, evidence)
    ):
        raise SourceError(
...
def _regular_file_stat_signature(path: Path) -> tuple[int, int, int, int, int]:
...
def _redaction_pairs(redactions) -> list[tuple[str, str]]:
    pairs: set[tuple[str, str]] = set()
    for raw, replacement in redactions or []:
        value = str(raw)
        if not value:
            continue
        pairs.add((value, str(replacement)))
        try:
            resolved = str(Path(raw).resolve())
        except (OSError, TypeError, ValueError):
            continue
        if resolved:
            pairs.add((resolved, str(replacement)))
    return sorted(pairs, key=lambda item: -len(item[0]))


def _sanitize_text(value: str, redactions, *, limit: int | None = None) -> str:
    text = value
    for raw, replacement in _redaction_pairs(redactions):
...
def _sanitize_value(value, redactions):
    ...
    if isinstance(value, dict):
        ...
        for key, child in value.items():
            key_text = str(key)
            sanitized_key = _sanitize_text(key_text, redactions)
            sanitized[sanitized_key] = (
                "<credential-redacted>"
                if _is_secret_key(key_text)
                else _sanitize_value(child, redactions)
            )
        return sanitized
    if isinstance(value, list):
        return [_sanitize_value(child, redactions) for child in value]
...
def _metadata_marker_survived(path: Path) -> bool:
    try:
    ...
    return len(comments) == 1 and (comments[0].text or "") == METADATA_MARKER


def _metadata_marker_present(path: Path) -> bool:
    try:
        root = ElementTree.fromstring(path.read_bytes())
    except (OSError, ElementTree.ParseError):
        return False
    return any(
        ...
        and (element.text or "") == METADATA_MARKER
        for element in root.iter()
    )


def _module_marker_survived(path: Path) -> bool:
    try:
        text = path.read_bytes().decode("utf-8-sig")
    except (OSError, UnicodeDecodeError):
        return False
    marker_index = text.find(BSL_MARKER)
    method_index = text.find("ЛинияПоддержки")
    return (
        marker_index >= 0
        and text.count(BSL_MARKER) == 1
        and method_index > marker_index
    )


def _module_marker_present(path: Path) -> bool:
    try:
        return BSL_MARKER in path.read_bytes().decode("utf-8-sig")
    except (OSError, UnicodeDecodeError):
        return False
...
def _new_flow_report(workspace: Path) -> dict:
...
def _finish_flow(report: dict, status: str, exit_code: int, redactions, failure=None):
    ...
    return exit_code, _sanitize_value(report, redactions)
...
def run_roundtrip_flow(
    ...
    *,
    ...
    report = _new_flow_report(workspace)
    try:
    ...
    preexisting_markers = []
    if _metadata_marker_present(catalog):
        preexisting_markers.append("metadata")
    if _module_marker_present(module):
    ...
    if not _payload_ok(baseline_build):
        return _finish_flow(report, "failed", 1, redactions, "baseline build failed")
    ...
    report["roundTrip"]["metadata"]["presentAfterMutation"] = (
        _metadata_marker_survived(catalog)
    )
    report["roundTrip"]["module"]["presentAfterMutation"] = (
        _module_marker_survived(module)
    )
    if not all(
        (
            report["roundTrip"]["metadata"]["presentAfterMutation"],
    ...
    report["roundTrip"]["metadata"]["absentBeforeFullDump"] = not (
        _metadata_marker_present(catalog)
    )
    report["roundTrip"]["module"]["absentBeforeFullDump"] = not (
        _module_marker_present(module)
    )
    if not all(
        (
    ...
    report["roundTrip"]["metadata"]["survived"] = _metadata_marker_survived(catalog)
    report["roundTrip"]["module"]["survived"] = _module_marker_survived(module)
    ...
    return _sanitize_value(
        {
            ...
            "sourceError": {"message": str(error)},
        },
        redactions,
    )


def _packaged_manifest_provenance(plugin_root: Path) -> dict:
...
def execute_gate(
    *,
    ...
    if report_target == parent_configuration_path:
        raise SourceError("report path must not overwrite the parent configuration input")
    temporary = None
    evidence = None
    ...
    try:
        plugin_manifest = _packaged_manifest_provenance(plugin)
        if evidence_dir is None:
            try:
                temporary = tempfile.TemporaryDirectory(prefix="unica-issue-76-")
            except OSError as error:
        ...
        command = [str(binary_path), *binary_args]
        ...
        parent_state_after = _regular_file_stat_signature(parent_configuration_path)
        ...
        inputs_unchanged = (
            database_state_before == database_state_after
            and source_state_before == source_state_after
            and parent_unchanged
        )
        report["provenance"] = {
            "unicaBinarySha256": _hash_file(binary_path),
        ...
        report["inputs"] = {
            "database": {
                ...
                "statUnchanged": database_state_before == database_state_after,
            },
            "sources": {
                "path": "$SOURCE_INPUT",
                "copy": source_receipt,
                "statUnchanged": source_state_before == source_state_after,
            },
            "parentConfiguration": {
                "path": "$PARENT_CONFIGURATION_INPUT",
                "copy": parent_receipt,
                "statUnchanged": parent_state_before == parent_state_after,
                "hashUnchanged": parent_receipt["sha256"] == parent_hash_after,
            },
            "privateCopiesOnly": inputs_unchanged,
        }
        report["evidence"] = {
        ...
        report = _sanitize_value(report, redactions)
    except SourceError as error:
        exit_code = 2
    ...
    try:
        ...
        if temporary is not None:
            try:
                ...
                raise SourceError(f"cannot clean temporary issue #76 evidence: {error}") from error

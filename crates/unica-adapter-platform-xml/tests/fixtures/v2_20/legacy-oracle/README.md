...
This command does not rewrite either hand-authored public-contract specimen:
...
`NavigationEnvelope` and semantic properties, including all object kinds,
...
Rust tests build the corresponding values through public core types, use
wildcard-free matches for every public enum, compare exact canonical wires to
the static artifact, and strictly deserialize/round-trip every data-bearing
value/type union. The specimen is never regenerated by `--write`; `--check`
hash-verifies it through `oracle-manifest.json`.

...
fn run_meta_validation(
    ...
    let sessions = paths
        ...
        .map(|path| {
            ...
            crate::PlatformXmlAdapterFactory::new().capture_unscoped_validation_source(
                &target,
        ...
        if !keep_files {
            ...
            if remove_type_collection {
                ...
                if has_dir {
                    ...
                    transaction
                        .guard_pair_member_absent(obj_dir.clone())
                        .map_err(meta_remove_stdout_error)?;
                }
                if has_xml {
                    transaction
                    ...
                    transaction
                        .guard_pair_member_absent(obj_xml.clone())
                        .map_err(meta_remove_stdout_error)?;
                }
            }
...
pub(crate) fn validate_generated_metadata_boolean_contract(
    xml_text: &str,
    context: &str,
) -> Result<(), String> {
    let document = Document::parse(xml_text.trim_start_matches('\u{feff}'))
...
pub(crate) fn validate_generated_metadata_enum_contract(
    xml_text: &str,
    context: &str,
) -> Result<(), String> {
    let document = Document::parse(xml_text.trim_start_matches('\u{feff}'))
        ...
        .filter(roxmltree::Node::is_element)
    {
        ...
        let configuration_extension_compatibility_mode = "ConfigurationExtensionCompatibilityMode";
        if let Some(value) =
            meta_info_child_text(properties, configuration_extension_compatibility_mode)
                .filter(|value| !value.is_empty())
        {
            let allowed =
                super::cf::cf_validate_enum_allowed(configuration_extension_compatibility_mode);
            if !allowed.contains(&value.as_str()) {
                return Err(format!(
                    "{context} property {}.{configuration_extension_compatibility_mode} value '{value}' is not valid for the fixed 8.3.27 contract; expected one of: {}",
...
pub(crate) fn validate_semantic_metadata_artifact(
    ...
    let mut paths = std::collections::BTreeSet::from([object_path.to_path_buf()]);
    let owners = crate::infrastructure::platform_xml_owner::resolve_platform_xml_owners(
        object_path,
        workspace,
    )
    .map_err(|_| format!("{operation} metadata ownership evidence is unavailable"))?;
    paths.extend(owners.into_iter().map(|owner| owner.path));
    for path in paths {
        validate_semantic_metadata_artifact_path(&path, operation)?;
    }
    Ok(())
}

fn validate_semantic_metadata_artifact_path(
    object_path: &Path,
    operation: &str,
) -> Result<(), String> {
    let metadata = fs::symlink_metadata(object_path).map_err(|error| {
        format!("{operation} metadata artifact could not be read after publication: {error}")
    })?;
    if crate::infrastructure::platform::filesystem::metadata_is_link_or_reparse_point(&metadata)
        || !metadata.is_file()
    {
        return Err(format!(
            "{operation} metadata artifact is not a regular file after publication"
        ));
    }
    let raw = fs::read(object_path).map_err(|error| {
        format!("{operation} metadata artifact could not be read after publication: {error}")
    })?;
    let text = std::str::from_utf8(&raw).map_err(|error| {
        format!("{operation} metadata artifact is not UTF-8 after publication: {error}")
    })?;
    Document::parse(text.trim_start_matches('\u{feff}'))
        .map_err(|error| format!("{operation} metadata artifact is not valid XML: {error}"))?;
    validate_generated_metadata_boolean_contract(text, operation)?;
    validate_generated_metadata_enum_contract(text, operation)?;
    crate::versions::v2_20::validation::validate_metadata_value_contract(&raw, operation)
}
...
pub(crate) fn validate_meta_edit_preview(
    ...
    if let Some(definition_file) = definition_file {
        let definition_path = absolutize(definition_file, &context.cwd);

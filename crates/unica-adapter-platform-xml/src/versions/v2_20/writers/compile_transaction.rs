...
pub(crate) struct CompileTransaction {
    ...
    pair_member_absence_guards: BTreeSet<PathBuf>,
...
impl CompileTransaction {
    ...
    pub(crate) fn guard_semantic_check(
        ...
        self.guard_semantic_check_with_presentation(verify, false)
    }

    pub(crate) fn guard_semantic_check_preserving_error(
        &mut self,
        verify: impl Fn() -> Result<(), String> + Send + Sync + 'static,
    ) -> Result<(), String> {
        self.guard_semantic_check_with_presentation(verify, true)
    }

    fn guard_semantic_check_with_presentation(
        &mut self,
        verify: impl Fn() -> Result<(), String> + Send + Sync + 'static,
        preserve_error: bool,
    ) -> Result<(), String> {
    ...
    pub(crate) fn removes_path_or_ancestor(&self, path: &Path) -> Result<bool, String> {
        let identity = normalize_transaction_path_identity(path)?;
        Ok(self.removals.iter().any(|removal| {
            normalize_transaction_path_identity(&removal.path)
                .is_ok_and(|removal_identity| identity.starts_with(removal_identity))
        }))
    }
    ...
    pub(crate) fn guard_pair_member_absent(
        &mut self,
        path: impl Into<PathBuf>,
    ) -> Result<(), String> {
        let requested_path = path.into();
        let identity = normalize_transaction_path_identity(&requested_path)?;
        self.guard_path_absent(requested_path)?;
        self.pair_member_absence_guards.insert(identity);
        Ok(())
    }
        ...
        F: FnOnce() -> Result<(), String>,
    {
        ...
        let operation = (|| -> Result<CommitReport, String> {
            ...
            let mut final_guard_transients = prepared_creates
                .iter()
                .map(|(_, prepared)| prepared.staged_path().to_path_buf())
                .chain(
                ...
                .chain(
                    prepared_removals
                        .iter()
                        .map(|(_, recovery)| recovery.directory.clone()),
                )
                .collect::<Vec<_>>();
            ...
            while let Some((registration, prepared)) = prepared_registrations.pop_front() {
                ...
                final_guard_transients.push(recovery.directory.clone());

                let report = match prepared.commit_with_guard(|| {
                    if std::mem::take(&mut final_guard_pending) {
                        self.recheck_final_precommit_guards(&final_guard_transients)
                    } else {
                        Ok(())
    ...
    fn recheck_semantic_guards(&self) -> Result<(), String> {
        for guard in &self.semantic_guards {
            if let Err(error) = (guard.verify)() {
                return Err(if guard.preserve_error {
                    error
                } else {
                    "semantic source evidence changed after planning".to_string()
                });
            }
        }
        Ok(())
    ...
    fn recheck_final_precommit_guards(
        &self,
        transient_additions: &[PathBuf],
    ) -> Result<(), String> {
        self.recheck_exact_read_guards("after planning")?;
        self.recheck_absence_guards("after planning")?;
        self.recheck_directory_membership_guards(false, transient_additions, "after planning")?;
        self.recheck_semantic_guards()
    }

    fn recheck_absence_guards(&self, phase: &str) -> Result<(), String> {
        for path in &self.absence_guards {
            validate_absence_guard_with_label(
                path,
                phase,
                self.pair_member_absence_guards.contains(path),
            )?;
        }
        Ok(())
    }
    ...
    fn directory_membership_after_planned_deltas(
        ...
        for path in transient_additions {
            let metadata = fs::symlink_metadata(path).map_err(|error| {
                format!("failed to inspect transaction-owned publication artifact: {error}")
            })?;
...
fn validate_absence_guard(path: &Path, phase: &str) -> Result<(), String> {
    validate_absence_guard_with_label(path, phase, false)
}

fn validate_absence_guard_with_label(
    path: &Path,
    phase: &str,
    pair_member: bool,
) -> Result<(), String> {
    match fs::symlink_metadata(path) {
        ...
        Ok(metadata) => {
            let label = if pair_member || metadata.is_dir() {
                "pair member absence guard"
            } else {
                "absence guard"
            };
            Err(format!(
                "compile transaction {label} was violated {phase}: {}",
                path.display()
            ))
        }
        Err(error) => Err(format!(

...
impl fmt::Display for PublishError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match &self.kind {
            ...
            PublishErrorKind::PrecommitGuardRejected { message } => {
...
impl Error for PublishError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match &self.kind {
            ...
            | PublishErrorKind::PrecommitGuardRejected { .. } => None,
        }
    }
}
...
impl PreparedPublication<'_, '_, '_> {
    ...
    pub(crate) fn commit_with_guard(
        ...
        match self {
            Self::Unchanged => {
                guard().map_err(|message| {
                    PublishError::new(PublishErrorKind::PrecommitGuardRejected { message })
                })?;
                Ok(PublishReport {
            ...
            Self::Create(prepared) => prepared.commit_with_guard(guard),
            Self::Replace(prepared) => prepared.commit_with_guard(guard),
        }
    }
}
...
impl PreparedCreate<'_, '_, '_> {
    ...
    pub(crate) fn commit_with_guard(
        ...
        if let Err(message) = guard() {
            let mut error = PublishError::new(PublishErrorKind::PrecommitGuardRejected { message });
            attach_stage_cleanup(&mut error, &mut self.stage);
            return Err(error);
        }

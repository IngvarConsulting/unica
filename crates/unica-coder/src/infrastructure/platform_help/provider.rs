...
struct IndexedCorpus {
    ...
    /// Постинги лексического ядра: номера документов — позиции в
    /// `pages`.
...
/// смене ключа; на диск не пишется ничего.
...
impl PlatformSyntaxHelpProvider {
    ...
    /// Источник недоступен целиком — одна диагностичная секция.
    /// Её происхождение берётся у первого объявленного корпуса:
...
impl DocumentationProvider for PlatformSyntaxHelpProvider {
    ...
    /// текст прочитанной страницы, а не фрагмент. Индекс полного текста
...
mod tests {
    ...
    /// «есть, но не та»: выбор соседней версии был бы подменой.
    #[test]
    ...
    /// Лексический поиск: естественная формулировка — другой порядок слов и
    ...
    /// точного запроса того же имени: нечёткость понижает оценку.
    #[test]
    ...
    /// ним: индексы разных языков не должны смешиваться.
    #[test]
    ...
    /// `get` возвращает страницу целиком,

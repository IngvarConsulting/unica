//! Ретривал-гейт ADR-0037 п.6: golden-запросы задачи #415 против реальной
...
//! Замер #415 состоял из восьми запросов: пять до ADR-0037 отвечали пустой
//! секцией, они и составляют ядро набора. Восьмой запрос замера —
//! контрольный «Свернуть» — в recall-набор не входит: он проверял
//! относительный порядок внутри легитимно широкой выдачи, а не полноту.
...
/// Семь запросов замера #415: точные имена, естественные формулировки,
/// опечатка, английская пара имён. Контрольный «Свернуть» сознательно не
/// перенесён — см. шапку модуля.

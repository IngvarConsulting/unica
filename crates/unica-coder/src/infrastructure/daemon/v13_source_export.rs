...
use super::v13_infobase_exports::{
    digest_optional_workspace_file, digest_required_workspace_file, map_runner_code,
...
fn execute_with_resolved_runner(
    ...
    if prepared.if_rev.as_deref() != Some(revision.as_str()) {
        return reject(
            ...
            "source.export plan or environment changed after preview; run dryRun: true again",
        );
    }
    if cancellation.is_cancelled() {
        return reject(

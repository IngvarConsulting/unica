...
pub struct V8StdDocumentationProvider {
    ...
    /// Срок жизни кеша поиска стандартов в памяти процесса (ADR-0035 п.5).
...
impl DocumentationProvider for V8StdDocumentationProvider {
    ...
    fn search(
        ...
        // не вчерашним успехом (ADR-0035 п.5).
        let cached_body = {
...
mod tests {
    ...
    /// второго обращения к серверу (ADR-0035 п.5).
    #[test]
    ...
    /// запрещено», а не «ответим вчерашним» (ADR-0035 п.5).
    #[test]

...
//! пространства. Перевод на защищённое чтение актора отслеживается в #939.
//!
...
impl DocumentationProvider for ConfigurationHelpProvider {
    ...
    fn search(
        ...
        let versions: std::collections::BTreeMap<&str, Option<String>> = self
            ...
            .collect();
        // Матчинг — лексическим ядром. Корпус — страницы справки
...
mod tests {
    ...
    /// Терм в заголовке страницы весит больше терма в тексте:
    ...
    /// Лексический поиск: естественная формулировка с падежами находит страницу,
    ...
    fn help_is_found_at_every_legal_layout_location() {
        ...
        for (_, object) in &cases {
            ...
            // страница найдена и стоит первой.
            assert!(

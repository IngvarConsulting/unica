...
impl DocumentationProvider for ConfigurationHelpProvider {
    ...
    fn search(
        ...
        let versions: std::collections::BTreeMap<&str, Option<String>> = self
            ...
            .collect();
        // Матчинг — лексическим ядром (ADR-0035). Корпус — страницы справки
...
mod tests {
    ...
    /// Терм в заголовке страницы весит больше терма в тексте (ADR-0035):
    ...
    /// Ядро ADR-0035: естественная формулировка с падежами находит страницу,
    ...
    fn help_is_found_at_every_legal_layout_location() {
        ...
        for (_, object) in &cases {
            ...
            // страница найдена и стоит первой (ADR-0035).
            assert!(

//! Лексическое ядро поиска документации: токенизация,
...
//! независимость секций и локальность оценок остаются заботой вызывающего
//! кода. Состояния на диске нет — индекс живёт
//! там, где его построил вызывающий.
...
/// время жизни индекса — забота вызывающего.
...
/// признак, что спрошено именно это имя; точное совпадение ранжируется
...
/// (kb-1ci), когда установка доступна.

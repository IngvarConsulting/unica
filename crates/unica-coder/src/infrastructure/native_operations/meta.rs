...
use crate::domain::navigation_limits::{
    ...
    MAX_NAVIGATION_PROPERTY_SELECTORS,
    MAX_NAVIGATION_PROPERTY_VALUE_BYTES as SNAPSHOT_CACHE_MAX_PROPERTY_VALUE_BYTES,
    MAX_NAVIGATION_RELATIONS as SNAPSHOT_CACHE_MAX_RELATIONS, MAX_NAVIGATION_RELATION_SELECTORS,
...
const NAVIGATION_MD_NS: &str = "http://v8.1c.ru/8.3/MDClasses";
#[cfg(test)]
...
enum SnapshotCacheAdmission {
    Admitted(std::sync::Arc<crate::domain::navigation::NavigationEnvelope>),
    ResourceLimit,
}
...
impl SnapshotCache {
    ...
    ) -> Option<std::sync::Arc<crate::domain::navigation::NavigationEnvelope>> {
        self.entries
            .iter()
            .find(|entry| {
                entry.scope == scope
...
fn inspect_meta_navigation_inner(
    ...
    validate_navigation_public_input(args)?;
    ...
    let (navigation, target_ref, selection, cursor, include_cached_diagnostics) = match target {
        MetaNavigationTarget::ObjectPath(path) => {
            ...
            let navigation = match cache_ready_navigation_in(navigation, &binding, cache)? {
        ...
        MetaNavigationTarget::Cursor => {
            ...
            let value = args.get("cursor").expect("cursor target was parsed");
            crate::domain::navigation::NavigationCursor::authenticate(value, cursor_secret)?;
            ...
            let cursor = crate::domain::navigation::NavigationCursor::decode_authenticated(
                value,
                &snapshot.revision,
                &selection,
...
fn parse_meta_navigation_target(
    args: &Map<String, Value>,
) -> Result<MetaNavigationTarget, SourceAdapterError> {
    ...
    match (object_path, object_ref, snapshot_revision, cursor) {
        ...
        (None, None, None, Some(_)) => Ok(MetaNavigationTarget::Cursor),
...
fn exact_object<'a>(
    ...
    allowed: &[&str],
    label: &str,
) -> Result<&'a Map<String, Value>, SourceAdapterError> {
    ...
    SourceId::new(
        object
            ...
            .ok_or_else(|| {
                SourceAdapterError::new(
                    SourceAdapterErrorKind::DecodeCorrupted,
                    "objectRef has no valid sourceId",
                )
            })?,
    )
}
    ...
    ObjectKey::new(
        object
            ...
            .ok_or_else(|| {
                SourceAdapterError::new(
                    SourceAdapterErrorKind::DecodeCorrupted,
                    "objectRef has no valid objectKey",
                )
            })?,
    )
}
    ...
    SourceId::new(
        value
            ...
            .ok_or_else(|| {
                SourceAdapterError::new(
                    SourceAdapterErrorKind::DecodeCorrupted,
                    "navigation cursor has no valid sourceId",
                )
            })?,
    )
}
...
fn cursor_target_key(value: &Value) -> Result<ObjectKey, SourceAdapterError> {
    ObjectKey::new(value.get("target").and_then(Value::as_str).ok_or_else(|| {
        SourceAdapterError::new(
            SourceAdapterErrorKind::DecodeCorrupted,
            "navigation cursor has no valid target",
        )
    })?)
}

fn cursor_snapshot_revision(
    value: &Value,
    ...
    crate::domain::source_adapters::SourceRevision::new(
        value
            ...
            .ok_or_else(|| {
                SourceAdapterError::new(
                    SourceAdapterErrorKind::DecodeCorrupted,
                    "navigation cursor has no valid snapshotRevision",
                )
            })?,
    )
}
...
fn resolve_object_path_binding(
    path: &Path,
    context: &WorkspaceContext,
) -> Result<ResolvedConfiguredSourceBinding, SourceAdapterError> {
    ...
    let canonical_target = crate::infrastructure::source_roots::normalize_contained_source_root(
...
fn resolve_current_source_binding(
    source_id: &SourceId,
    context: &WorkspaceContext,
) -> Result<CurrentConfiguredSourceBinding, SourceAdapterError> {
    let source_map = discover_project_source_map(&context.workspace_root)
        .map_err(|_| source_unavailable("project source map cannot be resolved"))?;
    let config_path = source_map.config_path.clone();
    let configured_format_raw = source_map.configured_format_raw.clone();
    ...
    if source_set.source_format != crate::domain::project_sources::SourceFormat::PlatformXml {
        return Err(source_unavailable(
            "navigation source no longer authorizes the Platform XML adapter",
        ));
    }
    ...
    Ok(CurrentConfiguredSourceBinding {
        name: source_set.name,
...
fn cached_navigation_target(
    ...
    (
        std::sync::Arc<crate::domain::navigation::NavigationEnvelope>,
        ObjectRef,
    ),
    SourceAdapterError,
> {
...
fn parse_navigation_selection(
    value: Option<&Value>,
) -> Result<crate::domain::navigation::NavigationSelection, SourceAdapterError> {
    use crate::domain::navigation::{
        FacetSelection, NavigationSelection, PropertySelection, RelationKind, RelationSelection,
    };
    ...
    let properties = match object.get("properties") {
            ...
            for value in values {
                let name = value
                    ...
                    .ok_or_else(|| {
                        SourceAdapterError::new(
                            SourceAdapterErrorKind::DecodeCorrupted,
                            "select.properties values must be non-empty strings",
                        )
                    })?;
                if !names.insert(name.to_string()) {
                    return Err(SourceAdapterError::new(
                        SourceAdapterErrorKind::DecodeCorrupted,
                        "select.properties values must be unique",
                    ));
                }
            }
            PropertySelection::Named(names)
        }
        Some(Value::Object(value)) => {
            ...
            if names.is_empty() {
                return Err(SourceAdapterError::new(
                    SourceAdapterErrorKind::DecodeCorrupted,
                    "select.properties.named must not be empty",
                ));
            }
            ...
            for value in names {
                ...
                if !normalized.insert(name.to_string()) {
                    return Err(SourceAdapterError::new(
                        SourceAdapterErrorKind::DecodeCorrupted,
                        "select.properties.named values must be unique",
                    ));
                }
            }
            PropertySelection::Named(normalized)
        }
        _ => {
            return Err(SourceAdapterError::new(
                SourceAdapterErrorKind::DecodeCorrupted,
    ...
    let facets = match object.get("facets") {
        ...
        Some(_) => {
            return Err(SourceAdapterError::new(
                SourceAdapterErrorKind::DecodeCorrupted,
                "select.facets must be a string",
            ))
        }
    };
    let relations = match object.get("relations") {
        ...
        Some(Value::Array(relations)) => {
            ...
            for relation in relations {
                    ...
                    .any(|key| !matches!(key.as_str(), "role" | "kind" | "pageSize"))
                {
                    return Err(SourceAdapterError::new(
                        SourceAdapterErrorKind::DecodeCorrupted,
                        "select.relations item has unknown fields",
                    ));
                }
                let role = relation
                    .get("role")
                ...
                let kind = match relation.get("kind") {
                    ...
                    Some(Value::String(value)) => match value.as_str() {
                        ...
                        _ => {
                            return Err(SourceAdapterError::new(
                                SourceAdapterErrorKind::DecodeCorrupted,
                                "select.relations kind is invalid",
                            ))
                        }
                    },
                    Some(_) => {
                ...
                let mut selection = RelationSelection::new(role, page_size).map_err(|_| {
                    SourceAdapterError::new(
                        SourceAdapterErrorKind::DecodeCorrupted,
                        "select.relations role is invalid",
                    )
                })?;
                selection.kind = kind;
        ...
        _ => {
            return Err(SourceAdapterError::new(
                SourceAdapterErrorKind::DecodeCorrupted,
                "select.relations is invalid",
            ))
        }
    };
    crate::domain::navigation::normalize_navigation_selection(NavigationSelection {
        properties,
        facets,
        relations,
    })
    .map_err(|_| {
        SourceAdapterError::new(SourceAdapterErrorKind::DecodeCorrupted, "select is invalid")
    })
}
...
fn materialize_navigation_pages_with_secret(
    ...
    for relation_selection in &selection.relations {
        ...
        let matches_selection = |relation: &crate::domain::navigation::SemanticRelation| {
            relation.source == target
        ...
        let end = start
            ...
            .ok_or_else(|| {
                SourceAdapterError::new(
                    SourceAdapterErrorKind::DecodeCorrupted,
                    "navigation cursor position overflow",
                )
            })?
            .min(matching_len);
        ...
        let next_position = u64::try_from(end).map_err(|_| {
            SourceAdapterError::new(
                SourceAdapterErrorKind::DecodeCorrupted,
                "navigation cursor position cannot be represented",
            )
        })?;
    ...
    Ok(crate::domain::navigation::NavigationEnvelope {
        ...
        diagnostics: include_cached_diagnostics
            ...
            .unwrap_or_default(),
        relation_index: std::sync::Arc::clone(&navigation.relation_index),
    })
}

fn validate_navigation_public_input(args: &Map<String, Value>) -> Result<(), SourceAdapterError> {
    if let Some(select) = args.get("select") {
...
fn preflight_navigation_selection(value: &Value) -> Result<(), SourceAdapterError> {
    preflight_navigation_json(
        value,
...
fn preflight_navigation_json(
    ...
    serialized.map_err(|_| {
        SourceAdapterError::new(
            SourceAdapterErrorKind::DecodeCorrupted,
            format!("{label} cannot be serialized as JSON"),
        )
    })?;
    Ok(())
}
...
fn validate_navigation_json_structure(
    ...
    if depth > SNAPSHOT_CACHE_MAX_PROPERTY_VALUE_DEPTH {
        return Err(resource_limit(&format!(
...
fn project_selected_node(
    mut node: NavigationNode,
    selection: &crate::domain::navigation::NavigationSelection,
) -> NavigationNode {
    use crate::domain::navigation::{FacetSelection, PropertySelection};
    if let PropertySelection::Named(names) = &selection.properties {
        node.properties.retain(|name, _| names.contains(name));
    }
    match selection.facets {
        FacetSelection::None => {
            node.facet_visibility = crate::domain::navigation::NavigationFacetVisibility::None
        }
        FacetSelection::Summary => {
            node.facet_visibility = crate::domain::navigation::NavigationFacetVisibility::Summary
        }
        FacetSelection::Full => {
            node.facet_visibility = crate::domain::navigation::NavigationFacetVisibility::Full
        }
    }
    node
}
...
mod tests {
    ...
    fn meta_info_returns_ready_navigation_for_platform_xml_2_20() {
        ...
        let result = inspect_meta_navigation(
    ...
    fn meta_info_relation_cursor_preserves_child_nodes_and_changes_page() {
        ...
        let first = inspect_meta_navigation(
            &json!({
                "ObjectPath": path,
        ...
        let second = inspect_meta_navigation(
            &json!({"cursor": cursor}).as_object().unwrap().clone(),
            &context,
        )
        .unwrap();
        ...
        assert_ne!(
            first_page.items[0].object_ref,
            second_page.items[0].object_ref
        );
    ...
    fn every_target_mode_is_path_free() {
        ...
        assert!(!serde_json::to_string(&by_ref)
            .unwrap()
        ...
        let by_cursor = inspect_meta_navigation(
            &json!({"cursor": cursor}).as_object().unwrap().clone(),
            &context,
        )
        .unwrap();
        assert!(!serde_json::to_string(&by_cursor)
            .unwrap()
            .contains(&context.workspace_root.display().to_string()));
        std::fs::remove_dir_all(context.workspace_root).unwrap();
    }
    ...
    fn authenticated_cursor_rejects_recomputed_public_selection_hash_and_u64_max_position() {
        ...
        let group = first.relations[0].relation.clone();

        let mut forged = serde_json::to_value(&continuation).unwrap();
        forged["nextPosition"] = json!(2);
        forged["selectionHash"] = json!(crate::domain::navigation::normalized_selection_hash(
            &continuation.selection
        )
        .unwrap());
        let forged_result = inspect_meta_navigation(
            &json!({"cursor": forged}).as_object().unwrap().clone(),
            &context,
        )
        .unwrap();
        assert!(matches!(
            forged_result.status,
            crate::domain::navigation::NavigationStatus::Unavailable
        ));
        ...
        let overflow = crate::domain::navigation::NavigationCursor::issue(
            ...
            continuation.snapshot_revision.clone(),
        ...
        .unwrap();
        let overflow_result = inspect_meta_navigation(
            &json!({"cursor": serde_json::to_value(overflow).unwrap()})
                .as_object()
                .unwrap()
                .clone(),
            &context,
    ...
    fn retained_object_ref_absence_is_snapshot_stale() {
        ...
        let absent = inspect_meta_navigation(
            &json!({
                "objectRef": {"sourceId": root.source_id.as_str(), "objectKey": "uuid:00000000-0000-0000-0000-000000000000"},
                "snapshotRevision": snapshot.revision.as_str(),
            })
            .as_object()
            .unwrap()
            .clone(),
        ...
        .unwrap();
        assert_eq!(absent.diagnostics[0].code, "snapshot_stale");
        std::fs::remove_dir_all(context.workspace_root).unwrap();
    }

    #[test]
    fn object_ref_cache_miss_and_cross_scope_are_structured_unavailable() {
        let (first_context, first_path) = fixture(
            "2.20",
            "<Attribute><Properties><Name>Code</Name></Properties></Attribute>",
        );
        let first = inspect_meta_navigation(
            &json!({"ObjectPath": first_path})
                .as_object()
                .unwrap()
                .clone(),
            &first_context,
    ...
    fn continuation_scope_includes_workspace_epoch_and_source_set_kind() {
        ...
        let epoch_changed = WorkspaceContext {
            workspace_epoch: context.workspace_epoch + 1,
            ..context.clone()
        };
        let epoch_result = inspect_meta_navigation(&args, &epoch_changed).unwrap();
        assert!(matches!(
            epoch_result.status,
            crate::domain::navigation::NavigationStatus::Unavailable
        ));
        ...
        .unwrap();
        let kind_result = inspect_meta_navigation(&args, &context).unwrap();
        assert!(matches!(
            kind_result.status,
            crate::domain::navigation::NavigationStatus::Unavailable
        ));
    ...
    fn captured_object_path_binding_projects_original_bytes_after_source_mutation_and_retarget() {
        ...
        std::fs::copy(
            ...
            replacement.join("Catalogs/Items.xml"),
        )
        .unwrap();
        ...
        let error = cached_navigation_target(
            &snapshot.source_id,
            &target.object_key,
            &snapshot.revision,
            &context,
    ...
    fn relation_pages_group_explicit_roles_and_keep_edge_refs_unique() {
        ...
        let first = inspect_meta_navigation(&json!({
            "ObjectPath": path,
            "select": {"properties": ["name"], "facets": "none", "relations": [{"role": "attributes", "kind": "contains", "pageSize": 2}]}
        }).as_object().unwrap().clone(), &context).unwrap();
        ...
        let grouped = first
            .relation_index
            .iter()
            .filter(|relation| relation.group_ref.group_key == page.relation.group_key)
            .collect::<Vec<_>>();
        assert_eq!(grouped.len(), 2);
        assert_ne!(
            grouped[0].relation_ref.relation_key,
            grouped[1].relation_ref.relation_key
        );
        assert!(page
            .items
            .iter()
            .all(|item| item.properties.keys().collect::<Vec<_>>() == vec!["name"]));
        assert!(page.items.iter().all(|item| item.facet_visibility
            == crate::domain::navigation::NavigationFacetVisibility::None));

        let binding = resolve_object_path_binding(
        ...
        let tabular = full
            ...
            .find(|relation| {
                relation.source == target
                    && relation.role == crate::domain::navigation::RelationRole::TabularSections
            })
            .unwrap()
            .target
            .clone();
        ...
        reference_to_attribute.group_ref = crate::domain::navigation::RelationGroupRef::new(
            reference_to_attribute.source.source_id.clone(),
            reference_to_attribute.source.clone(),
            reference_to_attribute.role,
            reference_to_attribute.kind,
        )
        .unwrap();
        std::sync::Arc::make_mut(&mut full.relation_index).push(reference_to_attribute);
        ...
        .unwrap();
        let materialized = materialize_navigation_pages(&full, target, selection, None).unwrap();
        assert_eq!(materialized.relations.len(), 2);
        assert_eq!(
            materialized.relations[0].relation.kind,
            crate::domain::navigation::RelationKind::Contains
        );
        assert_eq!(
            materialized.relations[1].relation.kind,
            crate::domain::navigation::RelationKind::References
        );
        std::fs::remove_dir_all(context.workspace_root).unwrap();
    }
    ...
    fn relation_cursor_uses_stable_group_identity() {
        let (context, path) = fixture(
            "2.20",
            r#"<Attribute><Properties><Name>Code</Name><Synonym>Code synonym</Synonym></Properties></Attribute><Attribute><Properties><Name>Description</Name><Synonym>Description synonym</Synonym></Properties></Attribute>"#,
        );
        let first = inspect_meta_navigation(&json!({
            "ObjectPath": path,
        ...
        let page = &first.relations[0];
        assert_eq!(
            page.items[0].properties.keys().collect::<Vec<_>>(),
            vec!["name"]
        );
        assert_eq!(
        ...
        .unwrap();
        assert_eq!(
            second.relations[0].relation.group_key,
            page.relation.group_key
        );
        assert_eq!(
            second.relations[0].items[0]
                .properties
                .keys()
                .collect::<Vec<_>>(),
            vec!["name"]
    ...
    fn public_selection_limits_are_checked_before_normalization() {
        ...
        let properties = (0..MAX_NAVIGATION_PROPERTY_SELECTORS)
            .map(|index| Value::String(format!("property{index}")))
            .collect::<Vec<_>>();
        let relations = (0..MAX_NAVIGATION_RELATION_SELECTORS)
            .map(|_| json!({"role": "attributes", "pageSize": 25}))
            .collect::<Vec<_>>();
        let accepted = inspect_meta_navigation(
            &json!({
                "ObjectPath": path.clone(),
                "select": {"properties": properties, "relations": relations}
            })
            .as_object()
            .unwrap()
            .clone(),
        ...
        for select in [
            json!({"properties": (0..=MAX_NAVIGATION_PROPERTY_SELECTORS).map(|index| format!("property{index}")).collect::<Vec<_>>() }),
            json!({"relations": (0..=MAX_NAVIGATION_RELATION_SELECTORS).map(|_| json!({"role": "attributes"})).collect::<Vec<_>>() }),
            json!({"unknown": vec!["x".repeat(MAX_NAVIGATION_SELECTOR_STRING_BYTES); 600]}),
        ] {
            let result = inspect_meta_navigation(
                &json!({"ObjectPath": path.clone(), "select": select})
                    .as_object()
                    .unwrap()
                    .clone(),
                &context,
            ...
            .unwrap();
            assert_resource_limit_unavailable(&result);
        }
        std::fs::remove_dir_all(context.workspace_root).unwrap();
    }
    ...
    fn cursor_is_bounded_and_authenticated_before_selection_normalization() {
        ...
        let mut oversized =
            serde_json::to_value(first.relations[0].next_cursor.clone().unwrap()).unwrap();
        oversized["selection"]["relations"] = Value::Array(
            (0..=MAX_NAVIGATION_RELATION_SELECTORS)
                .map(|_| json!({"role": "attributes"}))
                .collect(),
        );
        let oversized_result = inspect_meta_navigation(
            &json!({"cursor": oversized}).as_object().unwrap().clone(),
            &context,
        )
        .unwrap();
        assert_resource_limit_unavailable(&oversized_result);

        let mut tampered =
            serde_json::to_value(first.relations[0].next_cursor.clone().unwrap()).unwrap();
        tampered["selection"] = json!({"properties": ["duplicate", "duplicate"]});
        let tampered_result = inspect_meta_navigation(
            &json!({"cursor": tampered}).as_object().unwrap().clone(),
            &context,
        )
        .unwrap();
        assert_eq!(tampered_result.diagnostics[0].code, "decode_corrupted");
        std::fs::remove_dir_all(context.workspace_root).unwrap();
    }
    ...
    fn response_shares_the_retained_relation_index_without_copying_pages() {
        ...
        let cache = Mutex::new(SnapshotCache::default());
        let response = inspect_meta_navigation_with_cache(
            &json!({
                "ObjectPath": path,
                "select": {"relations": [{"role": "attributes", "pageSize": 1}]}
            })
            ...
            &context,
            &cache,
            b"shared-relation-index-test-secret",
        )
        .unwrap();
        let cached = cache
            .lock()
            ...
            .navigation
            .clone();
        assert_eq!(response.relations[0].items.len(), 1);
        assert!(std::sync::Arc::ptr_eq(
            &response.relation_index,
            &cached.relation_index,
        ));
        std::fs::remove_dir_all(context.workspace_root).unwrap();
    }
    ...
    fn cache_preflight_accepts_a_twenty_five_thousand_node_identity_graph() {
        ...
        navigation.relation_index = std::sync::Arc::new(Vec::new());
    ...
    fn property_value_depth_limit_returns_resource_limit_without_recursion() {
        ...
        .err()
        .expect("depth limit + 1 must reject cache retention");
        assert_eq!(error.kind, SourceAdapterErrorKind::ResourceLimit);
    ...
    fn structured_property_larger_than_semantic_string_limit_is_cacheable() {
        ...
        for index in 0..600 {
            fields.insert(
                ...
                crate::domain::navigation::PropertyValue::String("x".repeat(1024)),
            );
        }
        navigation
            .nodes
            ...
            .find_map(|node| node.properties.values_mut().next())
            .unwrap()
            .value = Some(crate::domain::navigation::PropertyValue::Structure(fields));
        let snapshot = navigation.snapshot.as_ref().unwrap().clone();
        let entry = CachedNavigation::new(
    ...
    fn individual_semantic_string_larger_than_limit_is_not_cacheable() {
        ...
        navigation
            ...
            .find_map(|node| node.properties.values_mut().next())
            .unwrap()
            .value = Some(crate::domain::navigation::PropertyValue::String(oversized));
        let snapshot = navigation.snapshot.as_ref().unwrap().clone();
        let error = CachedNavigation::new(
        ...
        .err()
        .expect("individual semantic string must reject cache retention");
        assert_eq!(error.kind, SourceAdapterErrorKind::ResourceLimit);
        assert_resource_limit_unavailable(
            &crate::domain::navigation::NavigationEnvelope::unavailable(error),
        );
    ...
    fn cursor_resume_materializes_only_its_bound_group_and_keeps_full_selection() {
        ...
        for (index, relation) in attributes.iter().take(2).enumerate() {
            ...
            .unwrap();
            std::sync::Arc::make_mut(&mut full.relation_index).push(form);
        }
        let selection = parse_navigation_selection(Some(&json!({
            "properties": ["name"],
            "facets": "none",
        ...
        .unwrap();
        let initial =
            materialize_navigation_pages(&full, target.clone(), selection.clone(), None).unwrap();
        ...
        let attributes_second = materialize_navigation_pages(
            &full,
            target.clone(),
            selection.clone(),
        ...
        let attributes_third = materialize_navigation_pages(
            &full,
            target.clone(),
            selection.clone(),
        ...
        let forms_second = materialize_navigation_pages(
            &full,
            target.clone(),
            selection.clone(),
            Some(forms_cursor),
        ...
        .unwrap();
        let error =
            materialize_navigation_pages(&full, target, missing_selection, Some(missing_cursor))
                .unwrap_err();
    ...
    fn select_filters_properties_facets_and_relation_kind_at_runtime() {
        let (context, path) = fixture(
            "2.20",
            r#"<Attribute><Properties><Name>Code</Name></Properties></Attribute>"#,
        );
        ...
        let target = serialized["nodes"]
            .as_array()
            .unwrap()
            .iter()
            .find(|node| node["objectRef"]["displayName"] == "Items")
            .unwrap();
        assert_eq!(
            target["properties"]
                .as_object()
        ...
        for field in [
            ...
            assert!(
                ...
                "{field} must be absent for facets none"
            );
            assert!(
                serialized["relations"][0]["items"][0].get(field).is_none(),
                "{field} must be absent from relation items for facets none"
            );
        }
        assert_eq!(selected.relations.len(), 1);
        assert_eq!(
            selected.relations[0].relation.kind,
            crate::domain::navigation::RelationKind::Contains
        );
        ...
        let full = inspect_meta_navigation(
            &json!({
                "ObjectPath": context.workspace_root.join("src/Catalogs/Items.xml"),
                "select": {"facets": "full", "relations": [{"role": "attributes", "pageSize": 1}]}
            })
        ...
        let expected = std::collections::BTreeSet::from([
            ...
            "capabilityState".to_string(),
            "capability".to_string(),
            "actionProfile".to_string(),
            "semanticActions".to_string(),
            "actions".to_string(),
        ]);
        ...
        let runtime_error = parse_navigation_selection(Some(
            &json!({"relations": [{"role": "attributes", "offset": 0}]}),
        ))
        .unwrap_err();

...
\t\t\t</TabularSection>",
  ...
  "modify": {
    ...
    #[test]
    fn preview_meta_add_resource_generates_digit_aware_fallback_synonym() {
        let context = temp_context("add-resource-digit-aware-fallback");
        let object_path = context
            .cwd
            .join("InformationRegisters")
            .join("SampleStock.xml");
    ...
    #[test]
    fn preview_meta_rejects_top_level_synonym_outside_single_add_resource() {
        let context = temp_context("reject-out-of-scope-resource-synonym");
        let object_path = context
            .cwd
            .join("InformationRegisters")
            .join("SampleStock.xml");
        ...
        for (operation, value, expected) in [
            (
                "add-resource",
                "First: Number(15,2) ;; Second: Number(15,2)",
                "single Value item",
            ),
            ("add-ts", "Items", "only with add-resource"),
            (
                "modify-resource",
                "Amount: synonym=Total",
                "only with add-resource",
            ),
            ("remove-resource", "Amount", "only with add-resource"),
        ] {
            let mut args = meta_edit_args(&object_path, operation, value);
            args.insert("synonym".to_string(), json!("Visible value"));

            let outcome = preview_meta_edit(&args, &context);

            assert!(!outcome.ok, "{operation}: {outcome:?}");
            assert!(
                outcome.errors.iter().any(|error| error.contains(expected)),
                "{operation}: {:?}",
                outcome.errors
            );
            assert_eq!(fs::read_to_string(&object_path).unwrap(), original);
        ...
        let mut args = meta_edit_definition_args(&object_path, &definition_path);
        args.insert("Synonym".to_string(), json!("Visible value"));
        ...
        assert!(
            outcome
                .errors
                .iter()
                .any(|error| error.contains("not DefinitionFile")),
            "{:?}",
            outcome.errors
        );
        assert_eq!(fs::read_to_string(&object_path).unwrap(), original);
...
fn meta_edit_resource_auto_synonym(name: &str) -> String {
    if name.is_empty() {
        return String::new();
    }
    let mut result = String::new();
...
fn meta_edit_resource_synonym_arg<'a>(
    args: &'a Map<String, Value>,
    definition_file: Option<&Path>,
    operation: Option<&str>,
    value: &str,
) -> Result<Option<&'a str>, String> {
    let lowercase = args.get("synonym");
    let uppercase = args.get("Synonym");
    if lowercase.is_some() && uppercase.is_some() {
        return Err("Cannot use both synonym and Synonym".to_string());
    }
    let Some(raw_synonym) = lowercase.or(uppercase) else {
        return Ok(None);
    };
    let synonym = raw_synonym
        .as_str()
        .ok_or_else(|| "synonym must be a string".to_string())?;
    if definition_file.is_some() {
        return Err(
            "top-level synonym is supported only with Operation add-resource, not DefinitionFile"
                .to_string(),
        );
    }
    ...
    if operation != "add-resource" {
        return Err(format!(
            "{operation} does not accept top-level synonym; use it only with add-resource"
        ));
    }
    if split_meta_edit_batch_items(value, operation)?.len() != 1 {
        return Err(
            "add-resource accepts top-level synonym only with a single Value item".to_string(),
        );
    }
    Ok(Some(synonym))
}
...
fn edit_meta_with_mode(
    ...
    let edit_result = (|| -> Result<(MetaEditData, PathBuf, bool, Vec<String>), String> {
        ...
        let resource_synonym =
            meta_edit_resource_synonym_arg(args, definition_file.as_deref(), operation, value)?;
...
pub(crate) struct MetaEditInlineRequest<'a> {
    ...
    pub(crate) resource_synonym: Option<&'a str>,
}
    ...
    match action {
        "add" => {
            for item in split_meta_edit_batch_items(value, operation)? {
                ...
                if child_type == "resources" {
                    meta_edit_add_inline_resource(
                        xml_text,
                        object_type,
                        &item_value,
                        resource_synonym,
...
fn meta_edit_add_inline_resource(
    xml_text: &mut String,
    object_type: &str,
    value: &Value,
    synonym: Option<&str>,
    ...
    if attr.name.is_empty() {
        return Err("add-resource requires Value like Name: Type".to_string());
    }
    attr.synonym = synonym
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| meta_edit_resource_auto_synonym(&attr.name));
    validate_meta_compile_attr_type(&attr, "meta.edit add register field")?;
    meta_edit_ensure_top_child_name_free(xml_text, "Resource", &attr.name)?;
    let mut lines = Vec::new();
    let mut next_uuid = fresh_meta_compile_uuid;
    emit_meta_register_field(
        &mut lines,
    ...
    meta_edit_insert_top_child_object_with_position(xml_text, "Resource", &position, &lines)
}

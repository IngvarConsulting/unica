...
The [XML metadata read profile](../../../arch/rules/workspace/sources/metadata-read-profile.md)
uses fixtures for all 23 metadata kinds and independent property/collection
route matrices. This checks the reader profile, not the retired public
`meta.info` response. The fixture/profile guard is exercised by:
...
- Other path-based code readers preserve their

...
- **Rule:** `unica-bootstrap verify` объявляет установленное ядро годным
  ...
  скачивает уже опубликованный ассет ядра из выпуска GitHub, а отдельный шлюз
  проверяет достижимость всех ассетов движков и полный `prefetch`: зависание или неполный
...
### REQ-PERF-DELIVERY-WINDOW — Ожидание движка не истощает приём вызовов

- **Rule:** Первый вызов отсутствующего движка ждёт общую серверную доставку не
  более 30 секунд и заканчивает ожидание минимум за две секунды до известного
  срока хоста, а каждый присоединившийся вызов той же доставки отвечает состоянием без этого
  окна; отмена одного ожидателя не прерывает загрузку, поэтому 32 одинаковых
  холодных вызова не занимают все места допуска на время сети.
- **Decision:** ADR-0076
...
- **Rule:** Каждый инструмент возвращает один и тот же конверт верхнего уровня —
  ...
  `diagnostics`, `data`, `job` и `work`, поэтому новый адаптер добавляет подробности, не
  переделывая контракт, который разбирают вызывающие стороны.
- **Decision:** ADR-0001, ADR-0002, ADR-0076
- **Check:** `ci-test` — `crates/unica-coder/src/application/mod.rs`
- **Check:** `ci-test` — `tests/ci/test_unica_mcp_smoke.py`
- **Scope:** runtime
...
- **Rule:** Релиз собирает `unica` и `unica-bootstrap` для каждой поддерживаемой
  целевой платформы хоста, каждая цель проверяет собранный ею архив ядра, а
  манифест покрывает движками все те же цели прежде, чем ядро можно
  опубликовать; цель, провалившая собственную проверку, блокирует публикацию
  для всех остальных.
- **Decision:** ADR-0076
...
- **Rule:** Внутренний движок разрешается из пути артефактного кеша
  `<artifact>/<version>--<assetSha256>/<target>/<binaryPath>`, записанного в
  манифесте поставки, и проверяется по SHA-256 перед запуском, а упакованный bootstrap прогоняется в
...
- **Rule:** Второй запуск для той же пары «версия артефакта и SHA-256 ассета» и
  той же целевой платформы переиспользует уже опубликованный корень вместо
  повторной загрузки, а параллельные запуски публикуют его ровно один раз;
  другая сборка с той же семантической версией получает отдельный корень.
- **Decision:** ADR-0076
...
- **Rule:** Публичный пакет объявляет в своей записи `.mcp.json` собственный
  ...
  ядра по медленному каналу, чтобы первую установку версии не обрывало значение
  хоста по умолчанию; загрузка движков происходит уже после старта и этому
  бюджету не принадлежит.
- **Decision:** ADR-0076
- **Check:** `ci-test` — `tests/ci/test_package_unica_plugin.py`
- **Scope:** packaged, release

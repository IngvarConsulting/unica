...
Итого: решений 73, инвариантов 128, из них пересмотрено заново 111.

## Что приехало после заморозки

Слой заморожен на ветке архитектурного сброса. `main` о заморозке не знает и
продолжает писать в `spec/`, откуда правки приходят слиянием. Это история,
приехавшая с опозданием, а не переписанная здесь, поэтому она принимается как
есть и называется тут — иначе расхождение с моментом заморозки осталось бы
необъяснённым.

| Файл | Что пришло |
| --- | --- |
| `decisions/0074-primenennyy-runtime-ispolnyaetsya-v-ishodnom-vyzove.md` | новая запись: применённый runtime отвечает в исходном вызове |
| `decisions/0066-terminalnyy-receipt-runtime-v-odnom-vyzove.md` | отметка о том, что `ADR-0074` пересмотрел её предмет |
| `decisions/README.md` | строка `ADR-0074` в указателе |
...
| `ADR-0013` | Транспортом MCP владеет официальный Rust SDK | заменено на `CTR.WIRE.PROTOCOL` |
...
| `ADR-0016` | Единственный записываемый профиль выгрузки — платформа `8.3.27`, формат `2.20` | заменено на `CTR.FORMAT.PLATFORM-XML-8-3-27` |
...
| `ADR-0069` | Версии протокола MCP обслуживаются по документации SDK, гарантируются по матрице хостов | заменено на `CTR.WIRE.PROTOCOL` |
...
| `ADR-0074` | Применённый runtime исполняется в исходном вызове, а не отказывает | заменено на `DEC.2026-08-18.NO-JOB-REGISTRY`; классификация риска применённого вызова заново не пересмотрена |
## Инварианты
...
| `INV-MCP-SURFACE-SYNC` | Изменения публичной поверхности синхронны | заменено на `CTR.WIRE.TOOL-SURFACE` |

...
`info`), а чтения анализа кода выполняет только через внутренний MCP API
поставляемого процесса `rlm-bsl-mcp`. Единственный публичный MCP-сервер — `unica`
с инструментами `unica.*`. Оба исполняемых файла принадлежат апстриму и группе
релиза `rlm-tools-bsl`; текущее закрепление — `v1.33.0`, коммит

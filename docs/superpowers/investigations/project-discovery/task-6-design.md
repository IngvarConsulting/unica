# Task 6A/6B BSL Evidence Implementation Design
...
Task 6 начинается только после принятия Task 5A/5B и синхронизации active spec.
...
pub(crate) enum ProviderGapScope {
    ...
    QueryWide,
    SourceSetWide(String),                // exact linked source-set identity
}
...
`SourceSetWide` несёт exact linked source identity, когда file/catalog gap
...
pub(crate) struct CodeSearchQuery {
    ...
    pub(crate) max_records: u16,         // request.limits.maxEvidence
}
...
- `source_set` mismatch returns retryable `Unavailable(source_set_mismatch)`
  before any read.
...
- A forged query with a wrong kind, invalid cardinality/order/identity or a
  source string outside the existing `sourceSet` bound is a provider contract
  violation.
...
7. Map syntax facts to canonical ArtifactRefs, sort/deduplicate, enforce result
   limits, then construct typed `EvidenceRecord` with analysis source
   fingerprint and diagnostic workspace epoch.
...
| Port | name | version |
| --- | --- | --- |
| CodeSearchPort | `unica.snapshot_bsl_search` | `1` |
| DefinitionPort | `unica.snapshot_bsl_definitions` | `1` |
| CallGraphPort | `unica.snapshot_bsl_calls` | `1` |
...
- Any conditional branch (`#Если/#If`, `#ИначеЕсли/#ElsIf`, `#Иначе/#Else`,
  `#КонецЕсли/#EndIf`) is tokenized to find its balanced extent, but definitions
  and calls inside it are not promoted as positive facts. Add
  `conditional_compilation_unknown` to every affected definition/caller;
  unconditional definitions and calls outside the extent remain usable.
  CodeSearch may retain the literal source occurrence, but the same scoped gap
  prevents it from proving runtime reachability.
- Other directives add `unsupported_bsl_preprocessor`; no negative fact may be
  emitted for their affected scope.

### 6.6 Pure parser output contract
...
`comparison_text` is Unicode-lowercase for identifier/keyword/boolean/null
tokens and exact source spelling for number/punctuation tokens; it never
contains comments, strings, dates, deleted text or full source bytes.
...
placed in the positive vectors; their exact method spelling remains in a gap.
Shadow names are Unicode-lowercase, sorted and unique. Vectors are canonical:
tokens/calls by byte span, definitions by identity then name span, gaps by
reason/capability/method/span. `validate_against(byte_len)` checks every span,
index, closed variant, identifier/list/count bound and canonical order. It does
...
- identifier/keyword token comparison is Unicode lowercase;
- punctuation, numeric, boolean and undefined/null literals are exact;
...
For qualified resolution, the lexer recognizes `Перем`/`Var` declarations and
conservatively records possible receiver shadowing by a parameter, a module or
local variable declaration, or an identifier followed by assignment `=` in
the caller body. Any such observation downgrades the qualified receiver to
Dynamic. This deliberately permits false negatives; it forbids the false edge
that would result from treating a local object named like a CommonModule as
that CommonModule.

A queried caller must have exactly one unconditional definition. Zero callers
produce `missing_caller_definition`; duplicate or conditional callers produce
`ambiguous_caller_definition`. In both cases the CallGraph result is bounded
for that caller and emits no calls from that body.
...
- result records are fully canonicalized, then the smallest canonical prefix of
  `query.max_records` is retained;
...
| language subset | `unsupported_bsl_annotation`, `unsupported_bsl_preprocessor`, `conditional_compilation_unknown`, `unsupported_bsl_search_term`, `unsupported_bsl_call_syntax`, `unsupported_module_level_call` |
...
| deterministic resource | `bsl_file_limit`, `bsl_total_bytes_limit`, `bsl_file_bytes_limit`, `bsl_token_limit`, `bsl_nesting_limit`, `bsl_definition_limit`, `bsl_call_limit`, `bsl_result_limit` |
...
## 15. Corpus-seed dependency before Task 7

Task 6 unit fixtures are reusable micro-configurations, but they do not replace
the gold corpus. После GREEN Task 6B и **до wiring Task 7** создать RED corpus
seed from Task 13:

- strict `tests/fixtures/project_discovery/corpus.json` schema;
...
7, but schema validation and fixture references must pass. Task 7 must not be

...
> ветки.** Варианта `BadSignature` в `ContainerError` нет, и проверки первых
> четырёх байт файла (`bytes[..4] != 0x7FFFFFFF`) в `parse` нет тоже: это поле
> не сигнатура, а указатель на следующую свободную страницу. В установке
> 8.3.27.2074 ограничитель лежит там у 15 русскоязычных контейнеров из 38, а у
> остальных 23 — настоящее смещение внутрь тела, поэтому проверка отвергала 23
> контейнера из 38 как чужой формат и почти целиком обесценивала корпус
> `platform-guides` (386 страниц из 1 422). Формат опознаёт заголовок блока по
> смещению 16 — `\r\n`, три восьмизначных шестнадцатеричных поля и снова `\r\n`;
> его и проверяет `read_block_header`, возвращая `BadBlockHeader`. Замеры — в
> `docs/design/2026-08-08-platform-help-documentation-provider-design.md`.
...
{"en",0,0,"","GetURL(<Object>)"}
    ...
    fn reads_html_page_with_bilingual_signature() {
        let archive = zip_with(&[
            ("objects/Global context/methods/GetURL3758.html", PAGE.as_bytes()),
            ("objects/Global context/methods/GetURL3758.st", ST.as_bytes()),
        ]);
        ...
        assert_eq!(signature.ru.as_deref(), Some("ПолучитьНавигационнуюСсылку(<Объект>)"));
    ...
    fn reads_extensionless_page_by_markup() {
        let archive = zip_with(&[
            ...
            ("WebServices", "<HTML><BODY><H1>Web-сервисы</H1></BODY></HTML>".as_bytes()),
...
mod tests {
    ...
    fn mixed_language_installation_filters_by_language() {
        ...
        let dir = install(
            "8.3.27.2074",
            &["shcntx_ru.hbk", "shcntx_en.hbk", "1cv8_ru.hbk", "1cv8_en.hbk"],
        );
        let corpora = discover(&dir.path().join("8.3.27.2074"), "ru").expect("корпуса найдены");
...
```markdown
Заголовок записи: `INV-MCP-DOCUMENTATION-SECTIONS` — «Поиск по документации сохраняет независимые секции поставщиков»
...
(идентификатор записан не как настоящий заголовок `###`, чтобы пример в этом
одноразовом плане не столкнулся с реальной записью реестра в корпус-проверке
`test_identifiers_are_declared_once_in_the_whole_corpus`, — план не источник
истины, а сама запись уже добавлена в `spec/architecture/invariants.md`)

И в раздел `## APP`:

```markdown
Заголовок записи: `INV-APP-DOCUMENTATION-NO-DISK-STATE` — «Разбор корпуса справки не создаёт состояния на диске»
...
- [ ] **Step 2: Разделить ADR-0029 и принять её сузившуюся часть**

Запись писалась под весь замысел, а этот заход даёт его часть. Правила
каталога решений требуют переводить запись в `accepted` вместе с реализацией,
поэтому запись сужается до решённого и сделанного, а отложенное уходит в новую
запись. Так каждая из двух истинна в момент своего принятия.

**В ADR-0029 остаются** пункты 3, 4, 7, 8, 9, 10, 11, 13, 14, 17, 18 без
изменений — версия обязательна, отказ вместо подстановки соседней версии, текст
страницы как доказательство, контракт `DocumentationProvider` и его отделённость
от `CodeIntelligenceProvider`, различие поставщика и смысла источника,
`unica.documentation.search` секциями, семантика частичного отказа, отсутствие
состояния на диске, диагностичные статусы, единственный публичный сервер.

**Правятся под фактическую поставку:**

- пункт 1 — источник один: контейнеры справки установленной платформы. Второй
  источник назван замыслом, а не действующим решением;
- пункт 12 — реестр содержит поставщика справки установленной платформы;
  прочие поставщики отнесены к ADR-0030.

**Уезжают в ADR-0030 целиком:** пункты 2, 5, 6, 15, 16 — отбор корпусов базы
знаний, обход навигационного дерева, отношение к `robots.txt`, файл `unica.toml`
и правило его разбора.

Заголовок ADR-0029 приводится к сузившемуся охвату, статус — `accepted`, дата не
меняется. Раздел `Неграницы` дополняется пунктом: решение не вводит сетевых
источников документации и файла настройки — это предмет ADR-0030.

- [ ] **Step 2b: Завести ADR-0030 в статусе `proposed`**

Create: `spec/decisions/0030-setevye-istochniki-dokumentacii.md`

Запись несёт отложенное: базу знаний вендора вторым источником, обход
навигационного дерева от объявленных корней, неприменение `robots.txt` к отбору
страниц, перенос v8std за общий контракт с сохранением `unica.standards.*` как
совместимых фасадов, публичный аргумент сужения по смыслу источника, файл
`unica.toml` с правилом «неясность — отказ». Форма — по шаблону
`spec/decisions/README.md`: шапка со статусом и датой, затем `Контекст`,
`Решение`, `Неграницы`, `Последствия`, `Верификация`.

В разделе `Контекст` назовите, что предметное обоснование каждого пункта уже
записано в
[проектной записке](../../docs/design/2026-08-08-platform-help-documentation-provider-design.md)
и в ADR-0029, а эта запись отделяет отложенную часть от поставленной.

В `Верификации` для непринятой записи допустим один пункт `manual`, называющий,
что именно перечитывает ревьюер, — автоматических проверок у неё пока нет.

- [ ] **Step 2c: Обновить индекс решений**

В `spec/decisions/README.md` перенесите строку ADR-0029 из раздела «Предложенные
решения» в конец раздела «Принятые решения», приведя заголовок к новому, и
добавьте в «Предложенные решения» строку ADR-0030. Текст-заглушку «Сейчас
предложенных решений нет» не возвращайте: предложенное решение теперь есть.

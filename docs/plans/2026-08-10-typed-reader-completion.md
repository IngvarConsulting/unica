...
- [ ] Add a table test proving all 73 tools map one-to-one to the ledger's four
  result values and every `mutating=false` tool maps to `Read`.
- [ ] Run the table test and observe failure because the executable categories
  do not exist.
- [ ] Add schema/validation tests proving every reader omits and rejects
  `dryRun`, while mutation schemas retain it.
- [ ] Run the schema/validation tests and observe the current common argument
  admits `dryRun` for readers.
- [ ] Add application finalizer tests with literal handler outcomes for missing
  `data`, textual duplicate, both violations, failed typed reads and non-typed
  reads.
- [ ] Run them and observe that the current application returns empty or textual
  success.
- [ ] Implement the closed categories, reader argument boundary and single
  postcondition; make native readers execute through the normal read path.
- [ ] Run the focused tests and keep every existing mutation preview test green.
...
- [ ] Add command contract tests proving default and explicit analyze always
  report `--format jsonl`; `json`/`jsonl` are migration aliases and all other
  formats or modes reject `format`.
- [ ] Run them and observe default analyze still permits console output.
- [ ] Add parser fixtures for clean, findings, file failure and zero files, with
  hand-derived typed payloads.
- [ ] Run them and observe failure because no parser exists.
- [ ] Add fail-closed fixtures for order, duplicate events/paths, unknown fields,
  invalid scalars/ranges/tags/paths and inconsistent totals.
- [ ] Add filtering/sorting/limit tests proving file failures bypass filters and
  output is independent of upstream discovery order.
- [ ] Implement the parser and projection without retaining raw JSONL lines.
- [ ] Add a process test that streams more than 1 MiB successfully and rejects a
  physical line over 8 MiB while continuing to drain the child pipes.
- [ ] Implement the general line drain on the existing `ManagedChild` lifecycle.
- [ ] Route analyze through the streaming runner and verify cancellation,
...
- [ ] Replace the warning-only regression test with a literal readiness matrix
  requiring `ok=false`, no `data`/stdout and the stable error prefixes.
- [ ] Run it and observe `Missing` currently returns `ok=true` plus a warning.
- [ ] Add a post-execution stale test proving helper output is discarded and the
  same readiness mapper is used.
- [ ] Run it and observe the workspace service currently collapses readiness to
  a string error.
- [ ] Preserve structured readiness across the service response and map it once
  in `RlmNavigationAdapter`.
- [ ] Verify ready `definitions=[]` remains a successful typed result and
...
- [ ] Change ADR-0043 to `accepted` and make it own the combined implementation
  rather than depending on a separate PR.
- [ ] Change the design delivery boundary to the single-PR decision explicitly
  approved by the user.
- [ ] Add the new executable check to `INV-MCP-TYPED-RESULT` without rewriting
  ADR-0023.
- [ ] Update the diagnostics skill with typed completion/error semantics.
- [ ] Run focused Rust tests after every red-green slice.

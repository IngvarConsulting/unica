...
- RLM с бюджетом вызывающей стороны не обрезается до 120 секунд; внутренний
  ...
  `timeout_nanos`, запись сервиса с `schema_version = 3` не переиспользуется, а
  умолчания обычных и управляющих путей сохраняются.
...
вызывающей стороны без дополнительного 120-секундного потолка, с полями
`timeout_seconds` и `timeout_nanos` протокола `schema_version = 4`; запись
сервиса с `schema_version = 3` не переиспользуется.

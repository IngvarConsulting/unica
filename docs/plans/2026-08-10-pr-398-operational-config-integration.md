...
сетевую политику ADR-0032, добавив операционные сроки ADR-0040 и устранив гонку
...
- `unica.toml` и `unica.local.toml` принадлежат общему контейнеру ADR-0032 и
  ADR-0040; второго config root нет.
...
- Рабочие операции анализатора и RLM с бюджетом вызывающей стороны не
  обрезаются до 120 секунд; внутренний протокол с `schema_version = 4` без
  потерь переносит `timeout_seconds` и `timeout_nanos`, запись сервиса с
  `schema_version = 3` не переиспользуется, а умолчания обычных и управляющих
  путей сохраняются.
...
- [ ] **Step 1: Зафиксировать одобренные design, ADR-0040 и этот план**
...
**Interfaces:**
- Produces: `INV-APP-CONFIG-SNAPSHOT` с владельцем ADR-0040 и
  `INV-APP-DOCUMENTATION-NETWORK-POLICY` со ссылками ADR-0032 + ADR-0040.
...
не как потолки, отдельный явный диапазон диагностики и рабочие операции
анализатора и RLM с бюджетом вызывающей стороны без дополнительного
120-секундного потолка, с полями `timeout_seconds` и `timeout_nanos` протокола
`schema_version = 4`; запись сервиса с `schema_version = 3` не
переиспользуется.
...
git commit -m "docs(config): закрепить общий контейнер ADR-0040"
```
...
соответствие ADR-0032/ADR-0040.

...
- Implement the approved contract in `docs/design/2026-08-12-logical-provider-neutral-diagnostics-design.md` and proposed ADR-0062 through ADR-0065; do not re-open the design while executing the plan.
...
- Before accepting ADR-0062 through ADR-0065, update from the actual target branch and renumber the proposed ADR files if another ADR number landed first.
...
- `INV-MCP-DIAGNOSTIC-TARGET` owned only by ADR-0063 for logical location/focus and absence of public absolute paths;
- `INV-APP-DIAGNOSTIC-PROVIDERS` owned only by ADR-0064 for registry order, provenance, independent failure, and no cross-provider deduplication;
- `INV-SOURCE-READER-MIGRATION` owned only by ADR-0065 for explicit `bridge|directSwitch`, with diagnostics named as the sole direct switch.

Keep RLM readiness in the existing typed-result/application rule but replace ADR-0045 ownership with ADR-0062. Do not merge these concerns into one new invariant.
...
Set ADR-0062 through ADR-0065 to `accepted`. On historical ADR-0045 change only the lifecycle metadata and add a resolvable replacement note naming ADR-0062, ADR-0063, and ADR-0064; do not rewrite its accepted decision text. Do the same minimal lifecycle update on ADR-0049 with ADR-0065. Update the decision index sections.
...
Update `INV-MCP-TYPED-RESULT`, `INV-APP-CONFIG-SNAPSHOT`, and relevant runtime prose from diagnostics `mode=analyze` to `action=analyze`. Keep ADR-0062 references limited to RLM readiness.
...
- ADR-0062 through ADR-0065 atomic ownership;

...
**Goal:** Привести публичную поверхность `unica` от 71 инструмента в трёх моделях обращения к семи входам модели `docs/design/2026-08-18-ideal-mcp-surface-design.md`, гейтуя каждый шаг замером «токены на решение» (#479).
...
| 5 | Переключение: 73 скилла переписаны на 7 входов, старые 71 сняты, мажорный релиз | ломающая, единственная | полный каталог задач на новой поверхности ≤ 50 % wire-стоимости baseline фазы 0; зонды на живых Claude Code и Codex зелёные |
...
**Фаза 5 (переключение).** Мажорный релиз по `docs/release-runbook.md`; переписывание 73 скиллов на 7 входов (механическая замена имён + пересмотр прозы выбора инструмента — прозы станет меньше по построению); снятие 71 инструмента из реестра; перегенерация ведомости до 7 записей; зонды на живых хостах (Claude Code, Codex) по протоколу зондов из памяти проекта; финальный замер полного каталога против baseline фазы 0.

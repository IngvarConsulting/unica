...
На старте Task 6 remote PR head был `e686f598`, а `main` — `d7818b5e`. Перед
финальной доставкой `main` продвинулся до `b0205624` и принял независимый
ADR-0039 про логическую связку подписки. Поэтому operational-решение до merge
перенумеровано в ADR-0040, после чего обе ветки изменений интегрированы без
повторного использования номера. Если head автора меняется, сначала нужно
интегрировать его новые коммиты и повторить проверку номеров.

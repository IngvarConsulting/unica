...
нельзя; новое решение ADR-0045 выбирает общую исполняемую границу и конкретные
...
явно потребовал доставить #297, #291 и #292 в одном PR; ADR-0045 объединяет
...
проект с четырёхзначным перечислением и transport-guard. ADR-0045 владеет
...
Эта коррекция входит в тот же implementation PR, что и ADR-0045. CI проверяет
...
Единый implementation PR ADR-0045 меняет:

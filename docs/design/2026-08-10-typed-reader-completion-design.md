...
нельзя; новое решение ADR-0043 выбирает конкретные протоколы двух readers, а
общая исполняемая граница должна прийти из исправленной реализации #297.
...
конкурирующий preview-контракт и применяется после #297.
...
проверяемого контракта. Исправление нельзя откладывать до ADR-0043: первый
implementation slice #297 должен перенести свой одобренный проект с
четырёхзначным перечислением и transport-guard. Он владеет категорией
исполнения, режимом вызова, реестром результата и общим финализатором. ADR-0043
после этого использует границу и владеет только JSONL и RLM-протоколами.
...
### Общий guard, затем два предметных протокола

Выбран этот вариант с явным порядком владения. Implementation PR #297 первым
доставляет live-реестр и application-finalizer. Последующий implementation PR
ADR-0043 переводит one-shot analyze через JSONL parser в устойчивую модель
Unica и сохраняет RLM readiness до публичной границы. Внутри ADR-0043 оба issue
остаются одним связным вертикальным срезом: они относятся к ложному успеху
typed-reader, но данные и отказ каждого поставщика не смешиваются.

## Предшествующий live-контракт #297
...
Эта коррекция входит в implementation PR #297, а не откладывается до реализации
ADR-0043. CI проверяет соответствие для каждого зарегистрированного
инструмента; `scope` остаётся отдельной ортогональной декларацией и также
сверяется с `tool-surface-review.json`. Rust владеет исполняемым видом
результата, JSON — ревью-состоянием и сценариями, а исчерпывающий parity-test не
допускает неявного many-to-one преобразования.

После handler и до построения публичного `OperationResult`
application-finalizer #297 проверяет каждый вызов
...
удаление именно из этой работы. Отдельная #297 владеет сужением публичной схемы
и не противоречит этой негранице. После неё reader не имеет режима preview,
ручной `dryRun` отклоняется до workspace/provider/backend, а каждый допустимый
`Read + Typed` проходит transport-guard. Реализация ADR-0043 этот механизм не
переопределяет и не добавляет второй finalizer.
...
Предшествующий implementation PR #297, а не ADR-0043, меняет
`application/mod.rs` и общий `application/tool_contracts.rs`: вводит категорию
исполнения, точный четырёхзначный result contract, transport-finalizer
`Read + Typed` и удаляет reader `dryRun`.

Implementation PR ADR-0043 меняет:
...
Текущий proposal PR закрывается без merge. После принятия #297 эти документы
переносятся в implementation PR, запись получает очередной свободный номер и
сразу статус `accepted`; `INV-MCP-TYPED-RESULT` ссылается на фактические
решения-владельцы общего guard и предметных протоколов.
...
До первой строки должен быть зелёным acceptance gate уже слитой #297:
четырёхзначный parity реестра, отклонение reader `dryRun` до
workspace/provider/backend и `TOOL_EXECUTION_ERROR` с префиксом
`typed_result_missing:` либо `typed_result_textual:` для нарушенного
`Read + Typed`. Этот gate не реализуется повторно в ADR-0043.
...
Этот PR фиксирует одобренный проект и proposed ADR-0043. Номер предварительный:
ADR-0041 и ADR-0042 заняты, а реализация #297 займёт следующий свободный номер
раньше. PR не меняет runtime, использует `Relates to #291` и `Relates to #292`,
не закрывает issue и не называется исправлением.

Proposal PR не сливается: после ревью он закрывается без merge. После доставки
#297 независимый implementation PR от обновлённого `main` переносит проектную
записку и ADR под очередным свободным номером сразу в статусе `accepted` вместе
с кодом, падавшими до исправления тестами, fixtures, skills, acceptance и
синхронизацией инварианта.

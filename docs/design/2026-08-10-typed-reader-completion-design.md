...
нельзя; новое решение ADR-0044 выбирает общую исполняемую границу и конкретные
протоколы двух readers в одном согласованном изменении вместе с #297.

Пока шло исследование, design PR #426 по #297 получил явное одобрение и был
закрыт без merge перед реализацией. Он выбирает
`ToolExecution::Read | Mutation`, режимы `Read | Preview | Apply`, удаление
`dryRun` у readers и постусловие успешного typed-read. Владелец проекта затем
явно потребовал доставить #297, #291 и #292 в одном PR; ADR-0044 объединяет
общую границу и предметные протоколы без конкурирующего preview-контракта.
...
проверяемого контракта. Объединённый implementation slice переносит одобренный
проект с четырёхзначным перечислением и transport-guard. ADR-0044 владеет
категорией исполнения, режимом вызова, реестром результата, общим
финализатором и связанными JSONL/RLM-протоколами.
...
Выбран этот вариант с единым владельцем. Один implementation PR доставляет
live-реестр и application-finalizer, переводит one-shot analyze через JSONL
parser в устойчивую модель Unica и сохраняет RLM readiness до публичной
границы. Оба issue относятся к ложному успеху typed-reader, но данные и отказ
каждого поставщика не смешиваются.

## Общий live-контракт #297

`ToolSpec` предоставляет исполняемый `result_contract()`, который исчерпывающе
и однозначно выводит четыре действующих значения ledger:
...
Эта коррекция входит в тот же implementation PR, что и ADR-0044. CI проверяет
соответствие для каждого зарегистрированного
...
`dryRun` у readers удаляется объединённой реализацией #297. Reader не имеет режима preview,
ручной `dryRun` отклоняется до workspace/provider/backend, а каждый допустимый
`Read + Typed` проходит единственный transport-guard.
...
update. Поэтому ADR-0044 не публикует recovery hint для `Missing`, `Stale`,
...
Единый implementation PR ADR-0044 меняет:

- `application/mod.rs` и `application/tool_contracts.rs`: вводит категорию
...
Первым красным срезом становится общий gate #297: четырёхзначный parity
реестра, отклонение reader `dryRun` до workspace/provider/backend и
транспортная ошибка с префиксом `typed_result_missing:` либо
`typed_result_textual:` для нарушенного `Read + Typed`. После его реализации
предметные тесты diagnostics и RLM доказывают полезные данные и честные отказы.
...
Этот PR доставляет одобренный проект и accepted ADR-0044 вместе с реализацией
задач #297, #291 и текущей части #292. Он основан прямо на `main`, содержит общий
guard, предметные адаптеры, падавшие до исправления тесты, skill и синхронизацию
инварианта; зависимого или последующего implementation PR нет.

...
нельзя; новое решение ADR-0043 выбирает общую исполняемую границу и конкретные
...
явно потребовал доставить #297, #291 и #292 в одном PR; ADR-0043 объединяет
...
проект с четырёхзначным перечислением и transport-guard. ADR-0043 владеет
...
Эта коррекция входит в тот же implementation PR, что и ADR-0043. CI проверяет
...
Единый implementation PR ADR-0043 меняет:
...
Этот PR доставляет одобренный проект и accepted ADR-0043 вместе с реализацией

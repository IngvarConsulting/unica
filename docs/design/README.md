...
Требование шапки действует для новых записок. Записки, написанные до его
введения, перечислены в списке исключений `HEADERLESS_LEGACY` того же теста —
сегодня в нём все документы этого каталога, поэтому контракт шапки пока не
связывает ни одного существующего файла. Список может только сокращаться:
добавить в него запись — значит объявить новый долг, и это видно в диффе теста.
...
- [`2026-07-26-versioned-source-adapter-architecture.md`](2026-07-26-versioned-source-adapter-architecture.md)
  — каноническая историческая запись выбора версионированных семейств
  источников; действующим выбором владеет ADR-0019.
- [`2026-07-26-versioned-format-adapter-boundary-design.md`](2026-07-26-versioned-format-adapter-boundary-design.md)
  — проект физической границы core/application/семейного адаптера; Task 8 в
  нём остаётся на ревью.
- [`2026-07-25-object-navigation-model-prototype.md`](2026-07-25-object-navigation-model-prototype.md)
  — предшествующий прототип, явно помеченный как замещённый двумя документами
  выше.

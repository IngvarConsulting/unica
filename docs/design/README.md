# Проектные записки

Здесь лежат датированные проектные записки: как мы пришли к выбору, какие
варианты рассматривали и почему отвергли. Их пишет скилл `brainstorming`
(superpowers) на шаге «Write design doc»; каталог задан проекту в
[`AGENTS.md`](../../AGENTS.md) и переопределяет путь плагина по умолчанию.

**Архивный материал планирования, а не источник истины.** Записка отражает
состояние на свою дату и после реализации не обновляется. Она не нормирует
поведение системы — ни сейчас, ни в момент написания. Нормативный слой живёт в
[`arch/`](../../arch/README.md): решения — в `arch/decisions/`, проверяемые
правила — в `arch/invariants/` и `arch/contracts/`.

## Обязательная шапка

Каждая записка открывается тремя полями:

```markdown
- Date: `YYYY-MM-DD`
- Status: `draft` | `approved` | `superseded`
- Decision: `ADR-NNNN` | `none — no architectural contract changed`
```

Поле `Decision` — главное. Оно отвечает на вопрос, который иначе никто не
задаёт: породил ли этот проект архитектурное решение. `none` — это утверждение,
которое ревью может отклонить, а не значение по умолчанию. Перечень того, что
считается архитектурным контрактом, и порядок повышения проекта до записи
решения заданы в [`AGENTS.md`](../../AGENTS.md).

Формат проверяет `tests/ci/test_design_documents.py`.

Требование шапки действует для новых записок. Записки, написанные до его
введения, перечислены в списке исключений `HEADERLESS_LEGACY` того же теста —
сегодня в нём все документы этого каталога, поэтому контракт шапки пока не
связывает ни одного существующего файла. Список может только сокращаться:
добавить в него запись — значит объявить новый долг, и это видно в диффе теста.

## Документы, на которые опирается CI

Эти файлы удалять и переименовывать нельзя без правки тестов:

- `2026-07-23-platform-8-3-27-format-2-20-design.md` — закреплён по пути как
  тестовый вход и свидетельство профиля формата в
  `tests/ci/test_format_profile_contract.py` и
  `tests/dev/test_verify_8_3_27_platform.py`; действующий контракт находится в
  `arch/contracts/CTR.FORMAT.PLATFORM-XML-8-3-27.md`.
- `2026-07-24-updatable-donor-parity-relations-design.md` — назван как
  свидетельство в `tests/fixtures/unica_mcp_script_parity/donor-relations.json`
  и в записях `docs/provenance/reviews/`.

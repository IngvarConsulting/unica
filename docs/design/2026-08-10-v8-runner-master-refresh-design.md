...
Исторический релиз Unica `v0.11.0` и текущий `main` (package version
`0.12.0` в обоих plugin manifests) закрепляют один снимок
`v8-runner`: `72d346c0a8fcf8373d9388257d11e6bef0ad70b2`. Актуальный на
момент решения upstream `master` —
`7ce1b062843d86644fe55741dbe0ee79f7ca767d`; он содержит 36 последующих
коммитов, но не содержит исправления private per-IB
...
Порядок не меняется: Unica должна ссылаться только на существующие и
проверенные байты. Опубликованный release/tag не перемещается и не

...
- ADR-0020 владеет адресами, корнями, `resolve/children`, местоположениями,
  селекторами создания и миграцией предметных инструментов;
- ADR-0021 владеет `resources/read/apply`, снимками, ресурсными handles и
...
- resource access остаётся обязательным направлением, но живёт в ADR-0021.
...
ADR-0020 является более новым решением: semantic replacement не называется
alias normalization.
...
- Resource API остаётся существенным риском, но его расширение не может
  незаметно изменить ADR-0020: им владеет ADR-0021.

...
мутации роли. Архитектурным выбором владеет ADR-0043; эта записка фиксирует
...
`ibcmd` 8.3.27.2074 хранит объектное право только со значением, отличным от
умолчания роли `setForNewObjects`, и не переживает опустевший блок `object`
(проверено импорт/экспорт-циклами на закреплённой платформе). Поэтому
`setRight` со значением умолчания удаляет элемент права вместе с его RLS и
опустевший блок без неизвестных детей; на отсутствующем праве или
неперечисленном объекте это no-op. Значение, отличное от умолчания, пишется
явно. Блок `object` writer не создаёт (`object_not_listed`), зависимости прав
платформы не моделируются — оба ограничения записаны неграницами ADR-0043.
...
классифицирует общий каталог политик ADR-0031 — локальная копия списка корней
не создаётся.
...
Текущий публичный инвентарь содержит 69 случаев: 67 проверенных платформой
после слияния `main` и две ветки этого проекта —
`meta-edit-predefined-items` и `role-edit-set-right`. Первый сценарий одной
транзакцией устанавливает `CodeType=Number` и добавляет числовой `Code`, поэтому
закрепляет связь описателя с companion writer. Два независимых запуска
генератора прошли строгую загрузку корпуса с одинаковым нормализованным SHA-256
контракта:
`1c4afc7adf86cdb8a0e94c1f87e2166e4759387158317848746de0cee678bedf`.

Полный точный гейт `ibcmd 8.3.27.2074` по этому корпусу завершился `PASS`
для всех 69 контрольных пунктов, включая обе новые ветки; канонический
`setRight` хранит право только со значением, отличным от умолчания
`setForNewObjects`, и именно эту форму платформа возвращает без нормализации.

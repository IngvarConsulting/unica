...
на три актуальных сценария: `unica.view {}` проверяет фактический корень и
конфигурацию проекта, `unica.view {at: "main:Catalog.Items"}` читает
типизированное свойство `Comment`, а `unica.search` находит BSL в
`main:CommonModule.Shared`. Все три сценария проверяют `structuredContent`
до удаления каталогов, после удаления frontend cwd и после удаления daemon cwd.

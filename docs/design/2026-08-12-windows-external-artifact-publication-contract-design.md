...
`ERROR_PATH_NOT_FOUND` и запускает rollback. Это пользовательский дефект из
#310 и его EPF-воспроизведение #264.
...
этому запуску `.artifacts-stage-*`, `.artifacts-backup-*` и metadata-sidecar.
Второй запуск также доказывает, что старое содержимое target действительно
заменено.

...
## 2026-07-29 merge reconciliation

This archived design is the canonical record of the versioned-source-adapter
choice. The live rule owners are ADR-0019 and the `INV-SOURCE-*`/`REQ-*`
entries linked from `spec/architecture/`; this section records the relationship
without turning the design directory into a second normative layer.

The architecture supports many adapter families and many explicit versions
inside each family. The planned families include Platform XML exports, EDT
exports and workspaces, direct binary CF/CFE containers, direct 1C file
databases, and future source families. The required read pipeline is:
...
Each family crate owns its physical layout, schema signatures, parsers,
serializers, writers, probes, and version dispatch. Version modules are private.
Selection requires positive family and version evidence; unknown, ambiguous,
or future evidence fails closed and there is no nearest-version fallback.
Direct CF and FileDB adapters must keep binary pages, container offsets, table
formats, and storage handles inside their family crates.

Opened revision sessions are immutable, lazy, and opaque outside their family.
Physical paths, byte offsets, database handles, parser handles, and native
object keys remain private. Core exposes only closed semantic IDs, object
kinds, typed properties and value states, relations, facets, coverage, status,
and neutral diagnostics. An adapter that encounters an unknown meaningful fact
must return partial coverage with a neutral diagnostic; it may not silently
drop the fact or publish a native extension key.

The AI-facing request and response contract is JSON only. Continuations use
opaque authenticated base64url cursors bound to source, revision,
authorization, target, relation, selection, and position. There is no legacy
text compatibility path. Information completeness is at least the useful
legacy full and drill-down inventory, including rights, complete type
descriptions, children, relations, facets, and bounded opaque-content evidence.

Readers and writers are separate. Writers accept closed family-neutral semantic
commands and return closed typed lifecycle/results. Execution revalidates the
revision and capability immediately before mutation, fails closed when
authorability or coverage is not proven, and publishes atomically with typed
rollback and recovery. `unica-application` and application code in
`unica-coder` know only neutral ports; `unica-coder` is the composition and MCP
host, with BSL-only interpretation allowed in the host while artifact
resolution and publication remain behind neutral adapter ports.

### Tasks 1-12 status at merge

| Task | State on this branch |
| --- | --- |
| 1. Neutral format core | Implemented |
| 2. Platform XML family crate | Implemented |
| 3. Format-neutral application orchestration | Implemented |
| 4. Closed semantic vocabulary | Implemented |
| 5. Platform XML semantic parity | Implemented for the reviewed Platform XML slice |
| 6. Specialized relation graph | Implemented for the reviewed Platform XML slice |
| 7. Read, guard, validation, publication ports | Implemented and previously reviewed |
| 8. Typed Platform XML writer boundary | **Under review; not complete or write-safe** |
| 9. Reduce `unica-coder` to composition/MCP host | In progress; no completion claim |
| 10. Enforce architecture boundaries | In progress; #257 registry integration is part of this merge |
| 11. Update public contracts and documentation | Pending final Task 8 review |
| 12. Full validation and PR completion | Pending; not run by this merge resolution |

Task 8 therefore does not promote the Platform XML adapter to `write-safe`.
EDT, direct CF, and FileDB implementations remain future work and are not
claimed by the existence of the common contracts.

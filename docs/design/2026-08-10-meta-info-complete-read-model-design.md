...
свойств и всех 23 видов метаданных, независимая алгебра наблюдаемых типов и
...
архитектурный выбор принадлежит ADR-0047. Новая полнота считается доставленной
только тогда, когда реализация, проверяемый инвариант, действующие ADR и
ведомость поверхности изменены вместе.

## Исходное состояние

### Что из #274 уже доставлено

Текущий `main` после #427, #325 и #428 больше не соответствует первоначальному repro
...
больше не описывает самостоятельный дефект runtime. Реализация ADR-0047 должна
...
- сохранение уже доставленного ADR-0042 контракта: `v8:UUID` наблюдается как
  вариант `uuid` с `mutationCapability: editable`, а неизвестный корректный
  QName остаётся локальным warning.
...
основном то, что умеет менять writer. ADR-0042 уже устранил такое сцепление для
типов дочерних элементов, но корневые свойства, kind-specific составные узлы и
маршруты `ChildObjects` всё ещё требуют отдельного read-профиля.
...
`ObservedMetadataType` описывает всё доказанное чтением в профиле
...
Алгебры чтения и мутации независимы: наблюдаемая может стать шире, но текущие
моделируемые варианты, включая UUID, имеют `mutationCapability: editable` по
ADR-0042. Writer получает `MetadataType` только явным fallible-преобразованием
или разбором публичной мутационной нагрузки; `readOnly` capability и нарушения
writer-ограничений при сужении отвергаются.

Один wire-parser разбирает `Type` в нейтральные наблюдаемые факты. Политика
чтения сохраняет все варианты; политика записи отдельно проверяет capability и
ограничения. Реализация сохраняет ADR-0042: UUID не деградирует в `null` и
...
Отслеживаемый manifest содержит ровно один основной исполняемый случай на каждый
из 23 видов и дополнительные edge fixtures для составных полей. Основной случай
строит канонический typed-шаблон и прямо называет соответствующий
`meta-compile-*` checkpoint exact platform corpus. Его порождённый XML не
копируется в 23 файла: такая копия была бы вторым результатом того же emitter-а,
а не независимым свидетельством. Guard утверждает одновременно:

- множество fixture-kind равно `MetadataKind::ALL`;
- каждый прямой семантический узел fixture классифицирован профилем;
- каждый объявленный профильный маршрут наблюдаем в canonical/edge fixture
  либо в независимой manifest-матрице маршрутов;
- `kind` и вариант `details` совпадают;
- неизвестное свойство или дочерний объект делает тест красным, а не исчезает
  из результата.
...
| `ChartOfAccounts` | accounting flags и extra-dimension accounting flags |
| `Task` | addressing properties и addressing attributes |
| четыре регистра | dimensions, resources, attributes и применимые properties |
| `Enum` | значения и доказанные свойства значения |

Edge fixtures берутся из официального дампа платформы или сверяются с уже
подтверждённым platform corpus. Если fixture расходится с действующей форматной
...
Исторический baseline design PR не является доказательством реализации. Итоговый
implementation PR проверяется заново поверх актуального `origin/main`, включая
полный workspace-набор, Python contract suites и architecture-sync.

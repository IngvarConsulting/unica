...
архитектурный выбор принадлежит proposed ADR-0040. Пока решение не принято
...
больше не описывает самостоятельный дефект runtime. Реализация ADR-0040 должна
...
7. Синхронизировать skill, acceptance, tool-surface review, ADR-0040 и новый
...
8. Проверить JSON-RPC: typed `structuredContent`, отсутствие `stdout`, raw XML и
   физических путей; после этого перевести ADR-0040 в `accepted` и закрыть

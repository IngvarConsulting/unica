...
- Статус проверки: текущая редакция сверена с полным обзором моделью `gpt-5.6-sol`; после любых изменений обязательна повторная перекрёстная проверка сильной моделью
...
| `Menestre1/reasoning-bank-poc` | Тестированное SQLite-хранилище опыта, продвижение по feedback и изоляция | В Unica нет доказанной политики долговременной reasoning-memory; право переноса не подтверждено | `deep-dive` |
...
| `alonehobo/1c-trusted-gateway` | Тестированные masking/type policy и наблюдаемые обходы approval | Нужен adversarial safety-тест; право переноса не подтверждено | `deep-dive` |
...
сравнительных чисел качества, задержки или размера. Текущая редакция проверена
на синхронность с английским evidence ledger моделью `gpt-5.6-sol`; после любых
изменений нужна повторная проверка. До любого решения о переносе также нужны
первичные результаты ограниченных экспериментов выше.

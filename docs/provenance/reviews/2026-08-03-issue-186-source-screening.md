...
- **Mechanism:** tests prove query read-only enforcement, BM25/indexing, reload and stale-content invalidation, installation, and event-log rights. Open/paid entitlement behavior and live infobase integration remain unverified.
- **Unica mapping:** adapt these semantics only behind `crates/unica-coder/src/infrastructure/native_operations/integration.rs` and `crates/unica-coder/src/application/operation_descriptors.rs`; expose existing `unica.runtime.execute`/`unica.code.search` contracts, never a second MCP server (`INV-MCP-NO-ENGINE-SERVERS`). Preserve preview/support guards, atomic publication, stable envelopes, and redaction. Event-log behavior is relevant to `plugins/unica/skills/log-analysis/SKILL.md`; state remains workspace-scoped.

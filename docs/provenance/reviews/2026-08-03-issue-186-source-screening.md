...
- **Evidence:** `crates/code-index-core/src/indexer/mod.rs` proves hash-based incremental reindex with new/changed/skipped/deleted counts, rayon parsing and sequential SQLite writes; daemon state/worker ownership is under `crates/code-index-core/src/daemon_core/` (`state.rs`, `worker.rs`, `server.rs`); federation dispatch is under `crates/code-index-core/src/federation/` (`dispatcher.rs`, `repos.rs`); MCP caps and unavailable/indexing responses are in `crates/code-index-core/src/mcp/tools.rs`. `crates/bsl-extension/tests/tools_integration.rs` is pinned test evidence; no crash-repair test was found. Evidence links resolve to `https://github.com/Regsorm/code-index-mcp/blob/9614acd9048f73fbc4379d5fd240dc457c8d957a/<path>`.

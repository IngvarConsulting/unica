...
> доступен на адаптере 0.11.2 только с `force:true` для полной замены одного
...
состоянием и защита от расхождения поколений принадлежат раннеру. Адаптер 0.11.2 разрешает `pull` и `push` только с `force:true`: `pull`

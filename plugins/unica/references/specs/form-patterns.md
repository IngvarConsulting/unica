...
> Адаптация правил дизайна элементов форм.
...
- Не используйте отрицательные заголовки: снятый флажок «Не проводить документ при записи» читается как двойное отрицание («не "не проводить"») — используйте положительную формулировку
...
- Ровно одна кнопка с `defaultButton: true` на форму
- Для других равнозначных и важных команд задавайте оформление вручную, не устанавливая `defaultButton`: `font.bold: true` и/или `backColor: "web:Yellow"`
...
### Подсказка (Hint)
- Через кнопку (`tooltipRepresentation: "Button"`) — для разовых подсказок новому элементу:
  ```json
  { "tooltip": "Пояснение для пользователя", "tooltipRepresentation": "Button" }
  ```
...
7. Не показывайте заголовок у поля Организация или Банковский счет для сокращения места (например, в шапке отчета)
8. Добавляйте подсказку ввода, если не показываете заголовок
9. Не используйте платформенный выбор типа для составных типов
10. Не делайте поле выбора для одного значения

### Флажок

1. Установите заголовок флажка справа
2. Используйте флажок для простой опции, в которой понятны оба состояния
3. Не используйте флажок для сложных опций — используйте переключатель
4. Не используйте в заголовке флажка отрицательные формулировки

### Выключатель (тогл, свитчер)

1. Используйте выключатель, если действие происходит сразу без подтверждений
2. Не используйте на одной странице больше 2–3 выключателей

### Переключатель

1. Используйте для 2–5 значений. Если больше — поле ввода со списком
2. Установите значение по умолчанию первым в списке
3. Выносите повторяющиеся тексты значений в заголовок

### Тумблер

1. Используйте для 3–5 значений
2. Не используйте для 2 значений

### Гиперссылки

1. Используйте для открытия подчинённых и служебных форм
2. Давайте осмысленное название
3. Не выполняйте действия на форме гиперссылкой — используйте кнопки
4. Не пишите «здесь», «тут», «подробнее»
5. Не ставьте две ссылки подряд

### Кнопка

1. Используйте для выполнения действия по запросу пользователя
2. Сделайте понятный заголовок
3. Не используйте кнопку для перехода к другой форме — используйте гиперссылку

### Табличная часть

1. Делайте заголовок табличной части в 1 строку
2. Уберите заголовок, если в ней всего одна колонка
3. Добавьте пустую последнюю колонку для управления шириной
4. Прижимайте заголовок колонки к значению

### Диалоговое окно

1. Опишите подробно, что требуется от пользователя
2. Опишите утвердительное действие в тексте кнопки
3. Обрабатывайте закрытие по крестику

### Подсказка

1. Используйте отображение через кнопку для разовых подсказок
2. Используйте подсказку сверху о всей форме, если она используется нечасто
3. Сделайте подсказку снизу для сложных элементов
4. Подсказок много не бывает, но они не должны быть навязчивыми

### Условное оформление

1. Применяйте одни и те же стили для аналогичных условий
...
При генерации формы через `unica.form.compile`:

1. **Цвета** — `"web:Yellow"`, `"#RRGGBB"` или `"auto"`. Не `RGB(r,g,b)`.
2. **Подсказки** — `tooltip` + `tooltipRepresentation`. Не `buttonHint`.
3. **Кнопка по умолчанию** — ровно одна `defaultButton: true`. Остальные — `font.bold: true` + `backColor: "web:Yellow"`.
4. **Заголовки флажков** — без отрицаний.
5. **Закрытие диалога** — `returnCode: "Cancel"`.

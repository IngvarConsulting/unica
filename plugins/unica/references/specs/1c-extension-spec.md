...
Канонический `unica.apply` с `code.insert`/`code.replace` и внутренний
`cfe.patch_method` изменяют BSL и дескриптор XML одной атомарной
транзакцией (`INV.SOURCE.CODE-BORROWED-MODULE-STATE`). При ошибке предусловия, несовместимом существующем

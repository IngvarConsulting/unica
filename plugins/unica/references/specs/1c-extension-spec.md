...
Наличие дочерних элементов не определяет наличие самого контейнера. Если класс
метаданных объявляет дочернюю коллекцию, дескриптор содержит `ChildObjects` даже
при пустой коллекции. В частности, заимствованные `Report` и `DataProcessor` без
форм и макетов всё равно заканчиваются пустым `<ChildObjects/>`; платформа
8.3.27 отвергает их дескрипторы без этого элемента.

Контейнер `ChildObjects` единственный, находится после `Properties` и имеет
namespace `http://v8.1c.ru/8.3/MDClasses`. Префикс XML может быть любым.

В текущей публичной поверхности MCP заимствование не опубликовано: старого
`unica.cfe.borrow` в `tools/list` нет, а словарь `unica.apply` не содержит
операции заимствования. Описанный формат и внутренний генератор сами по себе
не означают, что сценарий доступен через MCP.
...
Сохранённый внутренний обработчик перехвата метода изменяет BSL и XML одной
...
`Extended` считается идемпотентной. Повторное заимствование внутренним генератором сохраняет
существующие состояния свойств и не отключает уже подключённый модуль.

`unica.check` на корне набора типа `EXTENSION` (например,
`{"at":"ext:Configuration"}`) выбирает валидатор `cfe` и отвергает заимствованный объект, когда уже существует файл
прямого модуля, модуль владельца (`Ext/Module.bsl`, свойство `Module`) или модуль
команды (`Ext/CommandModule.bsl`, свойство `CommandModule`), а соответствующей
единственной записи `PropertyState` со значением `Extended` нет.

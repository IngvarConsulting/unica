...
`unica.apply` использует настоящий перевод строки для вновь синтезированного
...
точно как `v8:UUID`. Результат `unica.view` отделён от входа writer:
...
`unica.view` читает `Source`, `Event` и `Handler` обратно в то же логическое

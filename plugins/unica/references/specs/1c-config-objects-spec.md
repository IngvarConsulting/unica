...
Для ресурса существующего `InformationRegister`, который добавляет
`unica.meta.edit` операцией `add-resource`, профиль `8.3.27` задаёт такие
начальные значения:
...
Оба значения `Use` и `DontUse` допустимы в существующем XML; перечисленные
выше значения — умолчания операции добавления, а не ограничение формата.
При вставке в файл с единым LF или CRLF `unica.meta.edit` сохраняет BOM,
наблюдённый вид перевода строк и наличие завершающего перевода строки. Разделитель
между соседними элементами записывается переводом строки, а не сущностью
`&#13;` в содержимом XML.

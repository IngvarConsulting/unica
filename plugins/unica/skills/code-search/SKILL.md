...
- `scope` сужает поиск до одного логического поддерева. У локального текстового
  поиска `limit` задаёт размер страницы (по умолчанию 20, максимум 50): пока
  есть `cursor`, продолжай с теми же `query`, `scope`, `regex` и `limit`.
  `page.stoppedBy: "complete"` означает конец выдачи. После изменения исходников
  курсор отвечает `stale_cursor`; начни поиск заново. Поиск по именам и роли
  провайдеров пока не поддерживают продолжение.

...
Проверяет структурную корректность расширения: XML-формат, свойства, состав, заимствованные объекты, семантику сервисов (`HTTPService`, `WebService`) — включая соответствие `HTTPMethod` перечислению платформы 8.3.27 (`Any`, `GET`, `PROPFIND`, …). На канонической поверхности это `unica.check` на корне набора-расширения: валидатор `cfe` выбирается по виду набора, конфигурацию проверяет `cf`.

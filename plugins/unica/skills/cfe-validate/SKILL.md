...
Проверяет структурную корректность расширения: XML-формат, свойства, состав, заимствованные объекты, обязательный `ChildObjects`, согласованность файлов модулей (прямых, владельца, команды) с `xr:PropertyState=Extended`, семантику сервисов (`HTTPService`, `WebService`) — включая соответствие `HTTPMethod` перечислению платформы 8.3.27 (`Any`, `GET`, `PROPFIND`, …). Аналог `/cf-validate`, но для расширений.

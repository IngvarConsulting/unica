...
## Атомарная batch-правка одного модуля

Если изменение одновременно требует заменить caller и добавить или изменить
несколько независимых module-level helpers, не применяй их по одному и не
передавай целиком неизменённый большой модуль. Передай один `operations`
массив верхнего уровня вместе с общими `modulePath` и ровно одним
`sourceDir`/`sourceSet`. У каждого элемента массива те же обязательные поля
`selector`, `operation`, `content`, `expectedCount` и нужный `methodName`
или `anchor`.

Batch содержит минимум две операции. Все selector-диапазоны планируются по
одному исходному снимку, должны быть непересекающимися и целиком новыми либо
целиком уже применёнными; `module` selector внутри batch запрещён. Unica
строит final text в памяти, выполняет одну атомарную замену файла и возвращает
один общий diff/hash плюс per-operation cardinality. Частичное состояние,
пересечение или неоднозначность отклоняются без записи. Сначала делай
`dryRun: true`; после проверки применяй **тот же** batch и затем повторяй
dry-run, ожидая `noOp: true`.

Не смешивай top-level `selector`/`operation`/`content`/`expectedCount` с
`operations`: это разные контрактные режимы.

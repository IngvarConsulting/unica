...
- `sourceDir` — корень XML-исходников конфигурации внутри workspace.
...
- `selector: "methodDocs"` + `methodName` — выбрать непосредственно связанные с процедурой/функцией начальные `//`-комментарии и пустые строки. Включает проектные рамки `//+`/`//-`, если они входят в этот начальный trivia-блок; закрывающая `//-` после метода не выбирается. Не пересекает код или строки препроцессора и сохраняет аннотации; если документации нет, selector даёт cardinality `0`.
...
Передавать непустой `content`. Инструмент канонизирует переносы content, затем
рендерит их в EOL конкретного выбранного диапазона. LF, CRLF и их смешение в
одном модуле допустимы: для каждого изменения выбирается ближайший терминатор
после диапазона, затем перед ним, затем внутри него, иначе LF. BOM, terminal
newline и байты вне изменяемых диапазонов сохраняются; модуль целиком не
нормализуется. До записи planner моделирует следующий идентичный вызов: если
payload создаёт дополнительный anchor, скрывает anchor строкой/комментарием либо
меняет границы method selector, запрос отклоняется как недоказуемо
идемпотентный. Повтор принятого apply возвращает byte-identical no-op без
события изменения.
...
8. Повторить preview для проверки byte-identical no-op, затем запустить `unica.code.diagnostics`, тесты и безопасный build workflow по масштабу изменения.

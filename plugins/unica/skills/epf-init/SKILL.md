...
7. Предпросмотреть `unica.runtime.execute operation=make`; для applied-сборки в `v8project.yaml` потребуется доступная `infobase.connection`. Сам preview `.epf` не собирает.

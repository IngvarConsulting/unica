...
описание и структурный контекст, каталог `Subsystems` — дерево. `Mode`, `Name`,
`Limit` и `Offset` сняты: они выбирали срез печатного отчёта, которого больше
нет.
...
`Subsystem/ChildObjects` (ADR-0033, `INV-SOURCE-SUBSYSTEM-TOPOLOGY`). Для
...
инструмент возвращает ошибку вместо частичного дерева.

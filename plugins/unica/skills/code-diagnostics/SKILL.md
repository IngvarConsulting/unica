...
- Designer source-set `EXTERNAL_DATA_PROCESSORS` and `EXTERNAL_REPORTS` use the same read-only logical target flow for ObjectModule and FormModule; this does not widen `unica.code.patch`.

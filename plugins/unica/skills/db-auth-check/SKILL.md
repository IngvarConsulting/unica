---
name: db-auth-check
description: Используй для безопасной классификации уже полученного свидетельства об ошибке аутентификации или лицензии 1С, когда база нужна runtime workflow, а логин/пароль отсутствуют, неизвестны или не проходят. Applied runtime не запускай для проверки credentials.
...
`unica.build.*`.
- Не запускай auth probe самостоятельно, даже когда applied runtime доступен: это осознанная политика скилла, а не следствие ограничения runtime. Не выдавай чтение connection string или preview за успешное подключение к ИБ.
...
2. Если credentials явно заданы, не проверяй их preview: сообщи, что фактическая аутентификация не подтверждена.

...
```
Для одиночной `add-resource` можно передать верхнеуровневый
`synonym`/`Synonym`; значение записывается как есть. Без параметра
синоним генерируется из имени с разделением букв и цифр. Другие
операции, `DefinitionFile` и batch-режим с `;;` отклоняют верхнеуровневый
`synonym`.

...
`fillValue` поддерживается для `modify-attribute` и `modify-ts-attribute`, когда
свойство `FillValue` уже допустимо и присутствует в XML реквизита. `nil` или пустое
значение сбрасывает его в `xsi:nil="true"`. Ссылки вида

...
```
Верхнеуровневый `synonym`/`Synonym` можно передать одиночной операции
`add-resource`, `add-ts` или `add-ts-attribute`; значение записывается как есть.
Если параметр не передан, синоним генерируется из имени. В batch-режиме через
`;;` верхнеуровневый `synonym` отклоняется.

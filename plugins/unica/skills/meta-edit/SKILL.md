...
Для `modify-attribute` и `modify-ts-attribute` ключ `fillValue` изменяет только уже
существующее в XML свойство `FillValue`. Формат значения:
...
Если у реквизита нет свойства `FillValue`, операция завершается ошибкой без записи файла.

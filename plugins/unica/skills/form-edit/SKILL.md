...
Перед написанием BSL-кода посмотри структуру формы через `unica.form.info` и
найди все элементы, у которых `CommandName` указывает на нужную команду,
например `Form.Command.<ИмяКоманды>`. Если одна команда выведена в нескольких
местах, меняй все связанные элементы через
...
проверь свойство таблицы `ТолькоПросмотр`: когда таблица уже только для
просмотра, платформа обычно сама блокирует стандартные команды добавления,

...
- Весь batch атомарен: планирование, проверки ссылок и полная валидация спроецированного XML завершаются до фиксации транзакции. Apply дополнительно повторяет проверку формы (профиль `form` в `unica.check`) после записи внутри транзакции; при любой ошибке Form.xml не меняется.
...
4. `unica.check` с профилем `form` — проверить корректность

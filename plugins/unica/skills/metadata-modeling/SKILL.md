...
7. Verify statically with `unica.code.diagnostics`; use `unica.runtime.execute` only to preview typed syntax/test arguments, report runtime behavior as unverified, and search existing callers with `unica.code.search` when a type changed.

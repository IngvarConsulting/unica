...
отмена, тайм-аут или усечённый вывод повторную попытку не запускают. Если полная
повторная попытка тоже завершилась ошибкой, третьей попытки нет. Комплексная
переработка runtime/runner для v14 остаётся отдельной задачей, а этот временный
fallback её не заменяет.

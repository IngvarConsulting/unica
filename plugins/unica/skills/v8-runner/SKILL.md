...
Перед запуском `build` через `unica.runtime.execute` или
`unica.runtime.job.start` Unica проверяет выбранные Designer configuration
source-set-ы. Если в Platform XML обнаружена активная поддержка поставщика,
инкрементальная загрузка заранее заменяется полной и результат содержит
предупреждение: точное изменение не доказано безопасным для partial load.
Явный `sourceSet` ограничивает эту проверку выбранным набором.

Проверка привязана к основному `<workspace>/v8project.yaml`. Для build через
другой `config`, а также когда marker поддержки существует, но не читается,
используй `fullRebuild: true`; без него вызов завершится до запуска платформы с
этой подсказкой. Инкрементальный вызов всегда передаёт абсолютный канонический
путь к основному `v8project.yaml` и перед запуском повторно сверяет точный
SHA-256 его содержимого, а разрешающий ответ о поддержке связывается с точным
снимком свидетельства. Поэтому унаследованный `V8TR_CONFIG`, перенаправление
символьной ссылки или подмена относительного рабочего каталога не заменяют уже
проверенный проект. Явный full build marker не читает.

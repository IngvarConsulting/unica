...
- Если `tools-download` падает на `failed to fetch latest release … 403`, это анонимный лимит GitHub API, а не ошибка проекта. Не повторяй вызов по кругу и не выдавай его за отказ 1С: скажи пользователю причину прямо. Runner ходит в GitHub анонимно и токен из окружения не читает, поэтому обойти лимит на стороне Unica нельзя — остаётся подождать сброса лимита или положить артефакт в `tools.client_mcp.extension.artifact.path` вручную.

...
`stdout` синхронного вызова — это JSON, а `unica.runtime.job.logs` у долгого
build остаётся пустым до конца сборки: промежуточных строк не будет ни в
`stdout.log`, ни в `stderr.log`. Не жди их и не считай пустой лог признаком
зависания — смотри `phase` и `heartbeat` через `unica.runtime.job.status`.
Явный `fullRebuild: true` сохраняет обычный текстовый вывод раннера.

...
- Stale generated state after branch switch or rebase should preview `build` with `fullRebuild=true`; the applied run carries `runtime_risk_critical_non_abortable`.
- Missing project config, artifact, or managed tool payload may be previewed with `config-init`, `make`, or `tools-download`; their applied persistent writes run and carry `runtime_risk_publication_without_bounded_recovery`, so an interrupted run may leave partial output. Do not bypass them with a direct runner invocation.

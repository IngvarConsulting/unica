...
ошибке восстанавливает точный `config.toml`, прежние регистрации и legacy cache
в обратном порядке. Старые каталоги удаляются только после checksum-проверки
runtime, MCP `initialize` и обязательного `tools/list`. Путь резервной копии
печатается в отчёте.
